/******
1_measurement_from_visit_occurrence_INNER_lookup_join Query prepared at 2024-03-13 13:32:28.779563 by query_builder.py

retrieves variables from the following row_ids: 80||81||156
******/
...
SELECT
	...
	ROW_NUMBER() OVER(PARTITION BY po.visit_occurrence_id , variable_name ORDER BY m.measurement_datetime ASC) AS seq
FROM
	...
	INNER JOIN LoOkUp_ScHeMa.IC3_Variable_Lookup_Table L on (L.concept_id = m.measurement_concept_id AND L.variable_name IN ('asa_score', 'weight'))

WHERE
	--m.measurement_datetime BETWEEN vo.parent_visit_start_datetime AND vo.parent_visit_end_datetime
	m.measurement_date BETWEEN CONVERT(DATE,vo.parent_visit_start_datetime) AND CONVERT(DATE, vo.parent_visit_end_datetime)
	AND
