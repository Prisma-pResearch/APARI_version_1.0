/******
labs_from_before_visit_occurrence_to_visit_end_datetime_INNER_lookup_join Query prepared at 2024-03-13 13:32:28.791563 by query_builder.py

retrieves variables from the following row_ids: 85
******/
...
SELECT 
 ...
 FROM 
	...
	INNER JOIN LoOkUp_ScHeMa.IC3_Variable_Lookup_Table L on (L.concept_id = m.measurement_concept_id AND L.variable_name IN ('creatinine'))
 WHERE 
	m.measurement_datetime <= vo.parent_visit_end_datetime
	AND
...
SELECT 
 ...
 FROM 
	...
	INNER JOIN LoOkUp_ScHeMa.IC3_Variable_Lookup_Table L on (L.concept_id = m.measurement_concept_id AND L.variable_name IN ('creatinine'))
 WHERE 
	m.measurement_datetime <= visit_end_datetime
	AND
	C.cohort_definition_id = XXXXXX
	AND
	C.subset_id = YYYY;