...
def split_development_validation(df: pd.DataFrame,
    ...
    if split_type == 'longitudinal':
            ...
            for _, valueset in valuesets.iterrows():
                ...
                if val_percent > 0:
                    df.loc[df[unique_index_col].isin(rows[(rows.dt.date > max_dev_date) & (rows.dt.date <= max_val_date)].index),
                           'cohort'] = f'{project_name}_Validation_{vs_label}_{rows[rows.dt.date > max_dev_date].min().date()}_{max_val_date}'

                if test_percent > 0:
                    df.loc[df[unique_index_col].isin(rows[rows.dt.date > max_val_date].index),
                           'cohort'] = f'{project_name}_Test_{vs_label}_{rows[rows.dt.date > max_val_date].min().date()}_{rows.dt.date.max()}'
        ...
        # split without stratification
            ...
            max_val_date = max(pd.to_datetime('1970-01-02').date(), max_dev_date) if val_percent == 0 else df.iloc[floor(df.shape[0] * (dev_percent + val_percent)), df.columns.get_loc(time_index_col)].date()

            if val_percent > 0:
                df.loc[((df[time_index_col].dt.date > max_dev_date) & (df[time_index_col].dt.date <= max_val_date)),
                       'cohort'] = f'{project_name}_Validation_{df.loc[df[time_index_col].dt.date > max_dev_date, time_index_col].min().date()}_{max_val_date}'

            if test_percent > 0:
                df.loc[df[time_index_col].dt.date > max_val_date,
                       'cohort'] = f'{project_name}_Test_{df.loc[df[time_index_col].dt.date > max_val_date, time_index_col].min().date()}_{df[time_index_col].max().date()}'
