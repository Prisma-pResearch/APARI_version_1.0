...
def kruskal_wallace(input_df: pd.DataFrame, column_list: list, level_column: str, abbrev_delimeter: str = None) -> pd.DataFrame:
    ...
    grouped_df = input_df.groupby(level_column, group_keys=False).describe()
    ...
    output_df = cleanup_df(df=summary_df.groupby('column', group_keys=False).apply(format_summary_df).reset_index(),
                           index_col='levels')\
        .merge(cleanup_df(df=statistic_df.groupby('column', group_keys=False)
...
def _run_numeric_comparison(series: pd.Series, iid: bool = None) -> StatItem:
    ...
    iid: bool = iid if isinstance(iid, bool) else (shapiro(series).pvalue >= 0.05)
    groups: list = series.index.unique().tolist()
...
def summarize_groups(input_v: Union[dict, pd.DataFrame],
    ...
    for col in tqdm(columns, desc='Computing Comparisions'):
        ...
        if (col_dtype in ['str', 'sparse_int', 'cat_embedding', 'object', 'cat_one_hot']) and (len(std_dict) > 0):
