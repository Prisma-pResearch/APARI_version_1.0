...
def summarize_directory(directory: str,
    ...
    for batch in batches:
        for file in tqdm(file_list, desc='Summarizing Files'):
            print(f'{file}_{batch}')
...
def run_patient_census(directory: str, file_type: str = '.parquet', max_workers: int = 10, skip_daily_io: bool = True,**logging_kwargs) -> pd.DataFrame:
    pt_report = _make_patient_census(directory=directory, file_type=file_type, max_workers=max_workers, **logging_kwargs)
    if not skip_daily_io:
        daily_io_report = _process_dailyio_dask_census(directory=directory, file_type=file_type, **logging_kwargs)
    temp = []
...
def _process_patient_census_file(df: pd.DataFrame) -> pd.DataFrame:
...
def _process_dailyio_dask_census(directory: str, file_type: str, **logging_kwargs):
    
    return pd.concat([check_load_df(f, usecols=['patient_deiden_id'], tag_source=True,
                          max_workers=1, dtype=str, executor_type='ProcessPool', use_dask=True, allow_empty_files=True,
                          func_after_loading=_process_patient_census_file, **logging_kwargs).drop_duplicates().rename(columns={'source_file': 'source_batch'}) for f in tqdm(find_files(directory=directory, patterns=[r'daily_io_[0-9_]+\{}'.format(file_type)], regex=True), desc='Peforming Patient Census on Daily IO with Dask')], axis=0, ignore_index=True)

def _make_patient_census(directory: str, file_type: str, regex: bool = True, recursive: bool = False, use_dask: bool = False, max_workers: int = 10, **logging_kwargs) -> pd.DataFrame:
    ...
    for file in tqdm(detect_file_names(directory=directory, recursive=recursive, regex=regex, pattern=r'_[0-9_]+\{}'.format(file_type), omit_file_names=['daily_io', 'provider_info']),
        ...
        output[file] = check_load_df(file, directory=directory, patterns=[r'_[0-9_]+\{}'.format(file_type)], usecols=['patient_deiden_id'], tag_source=True,
                              ...
                              func_after_loading=_process_patient_census_file, **logging_kwargs).drop_duplicates().rename(columns={'source_file': 'source_batch'})
