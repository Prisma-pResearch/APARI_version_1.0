...
def get_credentials_v2(encrypted_dict_file_path: str, database: str, private_key_dir: str):
    ...
    elif database.lower() in ['idealist_omop', 'msr_data_sql_server', 'aki_epic_ii_omop', 'idealist', 'prismap_multimodal_ai', 'ic3_inpatient_pipeline', 'ic3_inpatient_pipeline_2024']:
...
def get_SQL_database_connection_v2(database: str,
                                   ...
                                   return_connection_url: bool = False):
    """
...
def execute_query_in_transaction(engine: Engine,
                                 query: str,
    ...
    **logging_kwargs : TYPE
    ...
    if str(type(engine)) == "<class 'sqlite3.Connection'>":
        ...
        try:
            ...
            if ('deadlocked' in exception_raised.lower()) and retry_on_deadlock:
                logm(message=f'Retrying: {query} after {retry_interval_sec} seconds', **logging_kwargs)
                retry = True
    ...
    if retry:
        return execute_query_in_transaction(engine=engine,
                                            query=query,
    ...
    elif raise_exceptions and (exception_raised != ''):
