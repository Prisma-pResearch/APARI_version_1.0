...
def p08_cal_mortality(encounter, pid: str):
    ...
    '''
    encounter['death_date_combined'] = encounter['death_date'].copy() if 'death_date' in encounter else None
    if 'ssdi_death_date' in encounter.columns:
    ...
    for x in ['admit_datetime', 'dischg_datetime', 'death_date_combined']:
        encounter.loc[:, x] = pd.to_datetime(encounter[x].values)
    ...
    con = (encounter['death_date_combined'].notnull()) & (encounter['death_date_combined'].dt.date
                                                          >= encounter['admit_datetime'].dt.date) & (encounter['death_date_combined'].dt.date <= encounter['dischg_datetime'].dt.date)
    encounter.loc[con, 'hospital_mortality'] = 'Y'

    if 'discharged_to_concept_id' in encounter.columns:
        encounter['dischg_disposition'] = encounter.discharged_to_concept_id.replace({'8863': 'TO SKILLED NURSING',
                                                                                      '4139502': 'TO HOME',
    ...
    con = (encounter['dischg_to_death_time'] <= 1) & (encounter['hospital_mortality'] == 'N') & ((encounter['dischg_disposition'] == 'EXPIRED') | (
        encounter['dischg_disposition'] == 'EXPIRED AUT') | (encounter['dischg_disposition'] == 'EXPIRED AUT UNK') | (encounter['dischg_disposition'] == 'EXPIRED NO AUT'))
    encounter.loc[con, 'hospital_mortality'] = 'Y'

    con = (encounter['dischg_to_death_time'] <= 7) & (encounter['hospital_mortality'] == 'N') & (
        (encounter['dischg_disposition'] == 'TO HOSPICE FACILITY') | (encounter['dischg_disposition'] == 'TO HOSPICE HOME'))
    encounter.loc[con, 'hospital_mortality'] = 'Y'
...
def p08_generateTrajectory(inmd_dir: str, in_dir: str, batch: int, eid: str, pid: str, patterns: list, var_file_linkage_fp: str, **logging_kwargs):
    '''
    ...
    None
    '''
    aki_episodes = check_load_df(os.path.join(inmd_dir, 'encounter_aki', 'encounter_aki_episodes_{}.csv'.format(batch)),
                                 pid=pid, eid=eid, dtype=None, **logging_kwargs)
    aki_summary = check_load_df(os.path.join(inmd_dir, 'encounter_aki', 'encounter_aki_summary_{}.csv'.format(batch)),
                                pid=pid, eid=eid, dtype=None, **logging_kwargs)
    ...
    encounter = check_load_df(os.path.join(inmd_dir, 'filtered_encounters', 'filtered_encounters_all_{}.csv'.format(batch)),
                              desired_types={'discharged_to_concept_id': 'sparse_int'},
                              pid=pid, eid=eid, dtype=None, **logging_kwargs)

    # read ssdi death
    if 'ssdi_death_date' not in encounter.columns.tolist():
        ssdi_death: pd.DataFrame = load_variables_from_var_spec(variables_columns=['death_date', 'death_type_concept_id'],
                                                               var_spec_key=var_file_linkage_fp,
                                                               data_source_key=in_dir,
                                                               project='AKI_Phenotype',
                                                               mute_duplicate_var_warnings=True,
                                                               patterns=patterns,
                                                               desired_types={'death_type_concept_id': 'sparse_int', 'death_date': 'datetime'},
                                                               allow_empty_files=True, regex=True, dtype=None, ds_type='pandas', **logging_kwargs)

        if 'death_type_concept_id' in ssdi_death.columns:
            ssdi_death.loc[:, 'death_type_concept_id'] = ssdi_death.death_type_concept_id.apply(lambda x: {'32817': 'death_date', '32885': 'ssdi_death_date'}.get(x, 'death_date')).values

            if ssdi_death.shape[0] == 0:
                ssdi_death = pd.DataFrame(columns=[pid, 'ssdi_death_date', 'death_date'])
            else:
                ssdi_death = ssdi_death.groupby([pid, 'death_type_concept_id'])['death_date'].min().reset_index(drop=False)\
                    .pivot(index=pid, columns='death_type_concept_id')['death_date']

        elif not isinstance(ssdi_death, pd.DataFrame):
            ssdi_death = pd.DataFrame(columns=[pid, 'ssdi_death_date'])
        encounter = encounter.merge(ssdi_death, on=pid, how='left')
    encounter = check_load_df(encounter,
                              use_col_intersection=True,
                              usecols=[pid, eid, 'admit_datetime', 'dischg_datetime', 'death_date', 'dischg_disposition', 'ssdi_death_date',
    ...
    aki_trajectory.loc[(aki_trajectory['paki'] == 0) & (aki_trajectory['aki_recovery'] == 0) & (aki_trajectory['aki_overall'] == 1)
                       & (aki_trajectory['dischg_place'] == 'Home/Rehab'), 'overall_type'] = 'non-pAKI with recovery'
    ...
    save_data(df=aki_trajectory, out_path=os.path.join(inmd_dir, 'aki_trajectory', 'aki_trajectory_{}.csv'.format(batch)), index=False)
    save_data(df=encounter[[eid, 'death_date_combined', 'hospital_mortality', 'mort_status_30d', 'mort_status_90d', 'mort_status_6m',
                            'mort_status_1y', 'mort_status_2y', 'mort_status_3y', 'dischg_place']],
              out_path=os.path.join(inmd_dir, 'aki_trajectory', 'encounter_dischg_place_{}.csv'.format(batch)), index=False)
