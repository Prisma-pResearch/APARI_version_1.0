...
def p01_load_filter_file(in_dir: str, inmd_dir: str, var_file_linkage_fp: str, batch: str, patterns: list,
                         append_subject_id_type_if_missing: str, pid: str = 'person_id',
                         eid: str = 'visit_occurrence_id', **logging_kwargs):
    '''
    ...
    None
    '''
    # debug_inputs(function=p01_load_filter_file, kwargs=locals(), dump_fp='aki_p1_kwargs.pkl')
    # raise Exception('stop here')
    # locals().update(pickle.load(open('aki_p1_kwargs.pkl', 'rb')))
    encounters = load_variables_from_var_spec(variables_columns=['visit_start_date', 'visit_start_datetime', 'visit_end_date', 'visit_end_datetime', 'discharged_to_concept_id', 'discharge_to_concept_id'],
    ...
    labs: pd.DataFrame = load_variables_from_var_spec(variables_columns=['creatinine'],
                                                      usecols=[pid, eid, "lab_id", 'variable_name', 'lab_type',
                                                               ...
                                                               'measurement_datetime', 'subject_id'],
                                                      var_spec_key=var_file_linkage_fp,
                                                      append_subject_id_type_if_missing=append_subject_id_type_if_missing,
                                                      ...
                                                      patterns=patterns,
                                                      allow_empty_files=True, regex=True, dtype=None, ds_type='pandas', **logging_kwargs)\
        .drop(columns=['subject_id'], errors='ignore')
...
if __name__ == '__main__':
    ...
    # from Python.Utilities.PreProcessing.data_format_and_manipulation import stack_df
    # from Python.Utilities.PreProcessing.clean_labs import clean_labs
