...
Created on Mon Jun 26 17:49:36 2023

source: https://github.com/akhtarshahnawaz/multiprocesspandas/blob/main/multiprocesspandas/applyparallel.py
...
def group_apply_parallel(self, func, num_processes=psutil.cpu_count(logical=False), n_chunks=None,
                         tqdm_desc: Union[str, None] = None, *args, **kwargs):
    """
    ...
    tqdm_desc: str = tqdm_desc if isinstance(tqdm_desc, str) else func.__name__
    func = functools.partial(func, *args, **kwargs)
    chunk_size = (len(self) // (n_chunks if n_chunks is not None else num_processes))

    with Pool(num_processes) as p:
        ret_list = p.map(func, tqdm([df.copy() for idx, df in self], desc=tqdm_desc), chunksize=max(1, chunk_size))
...
def series_apply_parallel(self, func, num_processes=psutil.cpu_count(logical=False), n_chunks=None,
                          tqdm_desc: Union[str, None] = None, *args, **kwargs):
    """
    ...
    with Pool(num_processes) as p:
        ret_list = p.map(func, tqdm(self.values.tolist(), desc=tqdm_desc), chunksize=max(1, chunk_size))
...
def df_apply_parallel(self, func, num_processes=psutil.cpu_count(logical=False), n_chunks=None,
                      tqdm_desc: Union[str, None] = None, axis=0, *args, **kwargs):
    """
    ...
    with Pool(num_processes) as p:
        if axis == 0:
            ret_list = p.map(func, tqdm([row for _, row in self.iterrows()], desc=tqdm_desc), chunksize=max(1, chunk_size))
        elif axis == 1:
            ret_list = p.map(func, tqdm([col for _, col in self.items()], desc=tqdm_desc), chunksize=max(1, chunk_size))
