...
from ..ProjectManagement.augment_var_spec_with_generated_varables import retrieve_augment_variable_specification
...
def make_tables_from_var_spec(instruction_fp: str,
                              ...
                              gen_var_link_patterns=[r'_[0-9]+_chunk_[0-9]+\.csv', r'_[0-9]+\.csv', r'\.h5', r'\.p'],
                              gen_var_link_use_regex: bool = True,
                              gen_var_link_exclusion_patterns: Union[List[str], None] = None) -> Dict[str, pd.DataFrame]:
    ...
    try:
        generated_data_link_df: pd.DataFrame = check_load_df(var_file_link_fp, sheet_name='gen_file_link')  # pd.read_excel(instruction_fp, sheet_name='Generated_Variables (Do Not Mod')
    except ValueError:
        generated_data_link_df: pd.DataFrame = retrieve_augment_variable_specification(spec_fp=instruction_fp,
                                                                                       project_name=project_name,
                                                                                       generated_data_dir=generated_data_dir,
                                                                                       recursive=True,
                                                                                       regex=gen_var_link_use_regex,
                                                                                       patterns=gen_var_link_patterns,
                                                                                       exclusion_patterns=gen_var_link_exclusion_patterns,
                                                                                       save_fp=var_file_link_fp)
    ...
    for table in summary_instructions.table_name.unique():
        ...
        for section in summary_instructions.table_section_header[table_idx].unique():
            ...
            for field_name in summary_instructions.table_field_name[section_idx].unique():
                ...
                preferred_source: str = summary_instructions.preferred_source[field_idx].iloc[0]

                assert project_variable_df.effective_name.isin([variable]).any() or generated_data_link_df.column_name.isin([variable]).any(), f'The requested variable: {variable} could not be found'

                project_var_idx: pd.Series = source_data_link.effective_name.isin([variable])
                if project_var_idx.any() and (pd.isnull(preferred_source) or (preferred_source in source_data_link.file_name[project_var_idx].tolist())):
                    ...
                    data: pd.DataFrame = check_load_df('', patterns=[(preferred_source if pd.notnull(preferred_source) else generated_data_link_df.source_file[generated_data_link_df.column_name == variable].iloc[0]) + '*.csv'], regex=False,
                                                       usecols=id_col_priority + [variable],
                                                       use_col_intersection=True,
                                                       directory=generated_data_dir,
                                                       recursive=True).drop_duplicates()
                try:
    ...
    out: Dict[str, pd.DataFrame] = _compile_results(results)

    if isinstance(out_path, str):
        save_data(out, out_path=out_path)
    return out
