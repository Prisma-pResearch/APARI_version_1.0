...
def _build_queries(df: pd.DataFrame,
                   ...
                   quick_audit_n: Union[int, None] = None,
                   save_audit_queries: bool = False,
                   use_visit_occurrence_parent_information: bool = False) -> List[str]:
    """
...
def get_table_and_concept_class(df: pd.DataFrame, engine_bundle: omop_engine_bundle) -> pd.DataFrame:
    ...
    # TODO: change the name of this table to avoid conflicts if multiple people are using this tool at the same time. Also drop the table when done
...
def _format_metadata(df: pd.DataFrame, engine_bundle: omop_engine_bundle, cohort_definition_id: int, cdm_version: str) -> pd.DataFrame:
...
def _solve_filter(row: pd.Series, time_index_mode: bool, engine_bundle: omop_engine_bundle) -> pd.Series:
    try:
        ...
        if (
                ((start_col == 'unbounded') and (end_col == 'unbounded'))
                or (row.cdm_table == 'person')
                or ((
...
def build_data_queries(df: pd.DataFrame, engine_bundle: omop_engine_bundle, cdm_version: str,
    ...
    template[['filters', 'index_bound', 'start_col', 'end_col']] = template.apply(_solve_filter, time_index_mode=time_index_mode, engine_bundle=engine_bundle, axis=1)
...
def build_query_group(dfg: pd.DataFrame, engine_bundle: omop_engine_bundle, cdm_version: str, use_visit_occurrence_parent_information: bool, time_index_mode: bool) -> str:
    ...
    if dfg.shape[0] == 1:
        ...
        return dfg.apply(build_query, engine_bundle=engine_bundle, cdm_version=cdm_version, 
                         use_visit_occurrence_parent_information=use_visit_occurrence_parent_information,
                         time_index_mode=time_index_mode, axis=1)
    else:
...
def _solve_field_join(row: pd.Series, use_visit_occurrence_parent_information: bool,  time_index_mode: bool) -> str:
...
def build_query(row: pd.Series, engine_bundle: omop_engine_bundle, cdm_version: str, use_visit_occurrence_parent_information: bool, time_index_mode: bool) -> str:
    # if (row.cdm_table == 'condition_occurrence') and (row.lookup_join_type == 'LEFT'):
    ...
    try:
        ...
        if use_visit_occurrence_parent_information:
            ...
            for ptrn in [r'\svisit_{}_datetime'.format(x) for x in ['start', 'end']]:
                query = re.sub(ptrn, ptrn.replace(r'\s', ' vo.parent_'), query)
                
            for ptrn in [r'\svisit_{}_date'.format(x) for x in ['start', 'end']]:
                query = re.sub(ptrn, ptrn.replace(r'\s', ' CONVERT(DATE, vo.parent_') + 'time)', query)
...
if __name__ == '__main__':
    ...
    #     build_query, build_data_queries, table_time_index_dict, build_query_group, _join_vars, _join_fields,  _join_row_ids, _test_writer, _solve_filter
    ...
    # validate_and_run_query_builder(spec_fp=r"Z:\GitHub\Ruppert_Thesis\Resource_Files\Ruppert_Thesis_Variable_Specification.xlsx",
    #                                     engine_bundle=engine_bundle,
    #                                     cohort_definition_id='visit_occurrence_id',
    ...
    #                                     use_visit_occurrence_parent_information=True)