...
def retrieve_augment_variable_specification(spec_fp: str,
    ...
    file_df = file_df.groupby('file_name', group_keys=False).agg({'cohort': deduplicate_and_join, 'source_file_path': 'first'}).reset_index(drop=False)
    ...
    for i, row in file_df.iterrows():
        ...
        if re.search(r'\.csv$', row.source_file_path):
            ...
            row_c['column_name'] = check_load_df(row.source_file_path).columns.to_list()
            row_c['key'] = 'N/A'
            rows.append(row_c)
    ...
    out: pd.DataFrame = file_df[['project_name', 'column_name', 'column_type_x', 'parent_category', 'child_category',
                                 'grand_child_category', 'source_file', 'cohort', 'key']].rename(columns={'column_type_x': 'column_type'})

    if isinstance(save_fp, str):
        if os.path.basename(save_fp) == 'variable_file_linkage.xlsx':
            vfl: pd.DataFrame = check_load_df(save_fp)
            outd: Dict[str, pd.DataFrame] = {'sql_file_link': vfl,
                                             'gen_file_link': out}
        save_data(df=outd, out_path=save_fp)
