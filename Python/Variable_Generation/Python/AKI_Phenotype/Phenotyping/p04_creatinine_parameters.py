...
def p04_create_creatinine_parameters(inmd_dir: str, eid: str, batch: int, pid: str):
    '''
    This function creates various creatinine parameters for each unique patient encounter.
    ...
    None
    '''
    encounter = load_data(os.path.join(inmd_dir, 'encounter_egfr_flags', 'encounter_egfr_flags_{}.csv'.format(batch)),
                          pid=pid, eid=eid)
    creatinine = load_data(os.path.join(inmd_dir, 'ckd_row_egfr', 'ckd_row_egfr_{}.csv'.format(batch)),
                           parse_dates=['inferred_specimen_datetime', 'admit_datetime'],
                           pid=pid, eid=eid)
    ...
    if not os.path.exists(os.path.join(inmd_dir, 'encounter_creatinine_parameters')):
        os.makedirs(os.path.join(inmd_dir, 'encounter_creatinine_parameters'))

    encounter.to_csv(os.path.join(inmd_dir, 'encounter_creatinine_parameters', 'encounter_creatinine_parameters_{}.csv'.format(batch)), index=False)
