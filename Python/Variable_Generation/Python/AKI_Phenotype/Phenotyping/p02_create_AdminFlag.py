...
def p02_create_admin_flags(inmd_dir: str, eid: str, pid: str, batch: int):
    '''
    This function creates patient history flag, specific history disease flags(aki, ckd, dialysis, kidneyTransplant, esrd) based on the patient history code.
    ...
    5. Use esrd_basic_codes & esrd_dialysis_codes & esrd_dialysis_aki_codes lists and aki_admin_flag & dialysis_admin_flag columns to locate ESRD condition in both diagnosis and procedure files. New columns introduced:
    ...
    diagnosis = load_data(os.path.join(inmd_dir, 'filtered_diagnosis', 'filtered_diagnosis_{}.csv'.format(batch)),
    ...
    procedure = load_data(os.path.join(inmd_dir, 'filtered_procedure', 'filtered_procedure_{}.csv'.format(batch)),
                          usecols=[pid, 'proc_date', 'proc_code',
                                     'proc_code_type'], parse_dates=['proc_date'],
                          pid=pid, eid=eid)\
    ...
    codes['code_date'] = codes['code_date'].dt.date
    ...
    cpt_codes = history_codes[history_codes['code_type'] == 'CPT']
    diagnose_codes = history_codes[history_codes['code_type'] != 'CPT']
    ...
    if not os.path.exists(os.path.join(inmd_dir, 'encounter_admin_flags')):
        os.makedirs(os.path.join(inmd_dir, 'encounter_admin_flags'))

    encounter.to_csv(os.path.join(inmd_dir, 'encounter_admin_flags', 'encounter_admin_flags_{}.csv'.format(batch)), index = False)
