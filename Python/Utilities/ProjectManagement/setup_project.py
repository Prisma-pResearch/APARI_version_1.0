...
def setup_OMOP_project_and_download_data(eligibility_query: str,
    ...
    if cdm_version == '5.3':
         query_replacements: dict = {**query_replacements,
                                     **{'admitted_from_concept_id': 'admitting_source_concept_id',
                                    ...
                                    '.parent_visit_start_datetime': '.visit_start_datetime',
                                    '.parent_visit_end_datetime': '.visit_end_datetime',
                                    '.parent_visit_occurrence_id': '.visit_occurrence_id'}}
    ...
    if query_mode in ['both', 'data_retrieval', 'build']:
    ...
    if not os.path.exists(cohort_success_fp):
        ...
        if not os.path.exists(raw_cache_success):
            logm(message=f'Running Eligibility Query: {os.path.basename(eligibility_query)}', display=True)
            save_data(df=check_load_df(input_v=eligibility_query,
                                       ...
                                       replacements={**{'XXXXTOP_NXXXX': f'TOP {n_samples}' if isinstance(n_samples, (int, str)) else ''},
                                                     **query_replacements},
                                       chunksize=1000),
    ...
    download_for_cohort(data_source=engine_bundle.engine,
                        ...
                        sql_file_list=validate_and_run_query_builder(spec_fp=configuration_instructions_fp,
                                                                     ...
                                                                     dir_dict=dir_dict,
                                                                     limit_to_cohorts=','.join(check_load_df(os.path.join(dir_dict.get('status_files'), 'cohort_ids.txt'), header=None).astype(int).astype(str).iloc[0, :]),
...
def setup_project(root_dir: str,
    ...
    if isinstance(copy_to_project, dict):
        for k, f in copy_to_project.items():
            for fi in (f if isinstance(f, list) else [f]):
                if ((fi.find(root_dir) == -1) and (k in dir_dict)):
                    logm(message=f'Copying {os.path.basename(fi)} to {k}')
                    _copy_files(fi, dir_dict.get(k))
                elif not (k in dir_dict):
                    logm(message=f'Skipping copying {k} to {os.path.basename(fi)} for {k} not in dir_dict', warning=True)
                else:
                    logm(message=f'Skipping copying {k} to {os.path.basename(fi)} for {k} file(s) are already in the project folder', warning=True)
...
def _ensure_folder_dict(folder_dict: dict, root_dir: str, non_unique_base_folders: list = []) -> dict:
    ...
    for k, v in flat_dict.items():
        ...
        tfp: str = os.path.join(root_dir.replace('\\\\', '\\'), k)
...
def download_for_cohort(data_source: Union[Engine, str],
    ...
    for sql_query in tqdm(sql_file_list, desc=f'Downloading data for {project_name}'):
        ...
        if not cohorts_to_process.completed.all():
            ...
            for cohort, row in tqdm(cohorts_to_process[~cohorts_to_process.completed].iterrows(),
                                    desc=f"Downloading: {os.path.basename(sql_query)}",
                                    total=cohorts_to_process[~cohorts_to_process.completed].shape[0]):
