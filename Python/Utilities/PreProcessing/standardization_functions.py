    ...
    ***Note*** this information can be derived from the transformation_instruction_fp.'''
    ...
    if use_existing_instructions:
        if os.path.exists(instruction_fp):
            logm(message=f'Loading Preprocessng instructions from file: {instruction_fp}', messageLevelName='INFO')
            training_df: pd.DataFrame = load_data(instruction_fp, dtype=None, messageLevelName='DEBUG')
    ...
    if pyt_seed:
        logm(message=f'Setting random seed with PyTorch Lightning to {random_seed}', messageLevelName='DEBUG')
        seed_everything(random_seed, workers=True)
    else:
        logm(message=f'Setting random and numpy random seed to {random_seed}', messageLevelName='DEBUG')
    ...
    if len(required_cols) > 0:
        logm(message=f'The following columns have been marked as required {required_cols}', messageLevelName='DEBUG')
    ...
    if isinstance(time_index_col, str):
        ...
        for c, m in zip(count_cols, missing_ind_cols):
            if bool(re.search(r'^1\.', pd.__version__)):
                out.loc[:, c] = out.loc[:, c].fillna(0).astype(int)
            else:
                out[c] = out.loc[:, c].fillna(0).astype(int)
...
def _process_column(series: pd.Series,
    ...
    **logging_kwargs : TYPE
    ...
    t_logging_kwargs.pop('messageLevelName', None)

    logm(message=f'Proccessing: {col_name}', **t_logging_kwargs, messageLevelName='INFO')
    ...
    if case_standardization != '':
        logm(message=f'Standardizing Case to: {case_standardization}', **t_logging_kwargs, messageLevelName='DEBUG')
        try:
    ...
    logm(message=f'Replacing replacement_values: {replacement_values}', **t_logging_kwargs, messageLevelName='DEBUG')
    default_other: str = replacement_values.get('xxxotherxxx',
                                                replacement_values.get('xxxotherxxx'.upper(),
                                                                       replacement_values.get('xxxotherxxx'.capitalize(),
                                                                                              config_dict.get('other_value'))))
    ...
    logm(message=f'Converting to {output_dtype}', **t_logging_kwargs, messageLevelName='DEBUG')
    ...
    logm(message='Calculating missingness', **t_logging_kwargs, messageLevelName='DEBUG')
            ...
            if ((out.dropna().nunique() == 1)
                ...
                if out.dropna().nunique() == 1:
                    logm(message=f"Dropping column: {col_name} all of the non-null values are the same",
                         **t_logging_kwargs, messageLevelName='DEBUG')
    ...
    if 'cat' in output_dtype:
            ...
            if min_cat_count != -999:
                ...
                if len(lower_than_theshold) > 0:
                    for v in lower_than_theshold.index:
                        logm(message=f'Replacing {v} with {rvalue}.', **t_logging_kwargs, messageLevelName='DEBUG')
        ...
        if len(unobserved_values) > 0:
            logm(message=f'Dropping {len(unobserved_values)} unobserverd values. Including: {unobserved_values}', **t_logging_kwargs, messageLevelName='DEBUG')

            r_candididates = config_dict.get('value_counts')[config_dict.get('value_counts').index.dropna()]

            final_r_value = (rvalue if rvalue in config_dict.get('levels') else
                             None if (r_candididates.shape[0] == 0) else
                             np.random.choice(a=r_candididates.index.values,
                                              replace=True,
                                              size=out.isin(unobserved_values).sum(),
                                              p=None if rvalue == 'xxxuniform_randomxxx' else (r_candididates.values
                                                                                               / r_candididates.sum())))
            logm(message=f'Filling unseen embedding levels {unobserved_values} with {"weighted random level from selection" if isinstance(final_r_value, np.ndarray) else ("other_value key: " + str(final_r_value))}',
                 **t_logging_kwargs, messageLevelName='DEBUG')
        ...
        if output_dtype == 'cat_one_hot':
            logm(message='One Hot encoding', **t_logging_kwargs, messageLevelName='DEBUG')
            ...
            if not training_run:
                for col in pd.Index([f'{col_name}_{x}' for x in config_dict.get('levels')]).difference(one_hot_df.columns).tolist():
                    logm(message=f'Filling unseen one_hot level {col} with zero', **t_logging_kwargs, messageLevelName='DEBUG')
                    one_hot_df[col] = 0

            return one_hot_df, config_dict

        elif ((output_dtype == 'cat_embedding') and (not skip_encoding_scaling)):
            logm(message='Creating Categorical Embedding', **t_logging_kwargs, messageLevelName='DEBUG')
        ...
        if isinstance(lower_upper, float):
            ...
            if replaceable_idx.any():
                logm(f'Replacing values less than {lower_limit_percentile:.2%} percentile with values between {lower_limit_percentile:.2%} and {fill_lower_upper_bound_percentile:.2%} percentiles',
                     **logging_kwargs, messageLevelName='DEBUG')
                ...
                logm(message=f'No values less than {lower_limit_percentile:.2%} percentile were found', **logging_kwargs, messageLevelName='DEBUG')
        else:
            logm(message=f'Clipping values less than {lower_limit_percentile:.2%} percentile', **logging_kwargs, messageLevelName='DEBUG')
        ...
        if isinstance(upper_lower, float):
            ...
            if replaceable_idx.any():
                ...
                if upper_lower == upper:
                    logm(message=f'Upper and upper lower values were the same. Imputing with {upper}', **logging_kwargs, messageLevelName='DEBUG')
                ...
                logm(message=f'No values greater than {upper_limit_percentile:.2%} percentile were found', **logging_kwargs, messageLevelName='DEBUG')
        else:
            logm(
                message=f'Clipping values greater than the {upper_limit_percentile:.2%} percentile', **logging_kwargs, messageLevelName='DEBUG')
...
def _calculate_stats_and_impute(series: pd.Series,
    ...
    **logging_kwargs : TYPE
    ...
    stat_dict: dict = {}
    if training_run:
        logm(message='Calculating Stats', **logging_kwargs, messageLevelName='DEBUG')
    ...
    if dtype in ['binary', 'cat_one_hot', 'cat_embedding']:
        if default_missing.lower() == 'xxxmodexxx':
            logm(message='Filling missing values using mode', **logging_kwargs, messageLevelName='DEBUG')
        ...
        if ((default_missing.lower() in ['xxxuniform_randomxxx', 'xxxweighted_randomxxx']) or ((default_missing not in config_dict.get('levels', [])) and (not training_run))):
            ...
            if (dtype == 'binary') and default_missing in [0, 1, '0', '1']:
                ...
                if default_missing.lower() in ['xxxuniform_randomxxx', 'xxxweighted_randomxxx']:
                    logm(message=f'Filling missing values using: {default_missing.lower()}', **logging_kwargs, messageLevelName='DEBUG')
                else:
                    logm(message=f'The default missing value ({default_missing}) was not in the training data. A weighted random sample from the training data will be used instead',
                         **logging_kwargs, warning=True, messageLevelName='DEBUG')
        ...
        if default_missing != '':
            ...
            if fillable:
                logm(message=f'Filling missing values using {default_missing}', **logging_kwargs, messageLevelName='DEBUG')
                ...
                if len(vc.index.values) > 0:
                    ...
                    logm(f'There were no samples to choose from: Leaving the column blank and dropping: {series.name}',**logging_kwargs, messageLevelName='WARNING')
                    stat_dict['drop_col_empty'] = True

    elif dtype in ['int', 'float']:
        if default_missing.lower() in ['xxxmean_std_normal_randomxxx']:
            logm(message='Filling missing values using values from a normal distribution matching the training data', **logging_kwargs, messageLevelName='DEBUG')
        ...
        if default_missing.lower() in ['xxxmedian_mad_normal_randomxxx']:
            ...
            logm(message='Filling missing values using values from a uniform distribution from the min/max values of the training data', **logging_kwargs, messageLevelName='DEBUG')
            ...
            logm(message='Filling missing values using mean', **logging_kwargs, messageLevelName='DEBUG')
            series.fillna(float(config_dict.get(
                'mean', stat_dict.get('mean'))), inplace=True)
            default_missing: str = ''
        elif default_missing.lower() == 'xxxmedianxxx':
            logm(message='Filling missing values using median', **logging_kwargs, messageLevelName='DEBUG')
            series.fillna(float(config_dict.get(
        ...
        if default_missing != '':
            try:
                ...
                logm(
                    message=f'Filling missing values using {default_missing}', **logging_kwargs, messageLevelName='DEBUG')
            except ValueError:
                logm(
...
def resample_pandas(data_structure: Union[pd.Series, pd.DataFrame],
                    ...
                    value_col: str = None,
                    serial: bool = False,
                    trim_based_on_start_end: bool = True,
                    subjects_per_batch: int = 500,
    ...
    value_col: str, optional
        ...
        The default is None.
    subjects_per_batch: int, optional
        The number of subjects to be placed in each batch if the total number of subjects is greater than the max-batch_size. The default is 500. This applies to auto_resample and the _integrate_and_trim_to_start_end_times functions.
    max_batch_size: int, optional
        The maxiumum number of subjects to be placed in a single batch. The default is 800. This applies to auto_resample and the _integrate_and_trim_to_start_end_times functions.
    **logging_kwargs
        kwargs to be passed to the log_print_email_message funciton in the Utils.log_messages module.
    ...
    """
    # debug_inputs(function=resample_pandas, kwargs=locals(), dump_fp='resample_kwargs.p')
    # raise Exception('stop here')
    logging_kwargs.pop('messageLevelName', None)
    if trim_based_on_start_end and (isinstance(start, (pd.Timestamp, pd.Series, pd.DataFrame)) or isinstance(end, (pd.Timestamp, pd.Series, pd.DataFrame))):
        logm(message='Trimming and resampling dataframe', **logging_kwargs, messageLevelName='INFO')
        data_structure: pd.DataFrame = _integrate_and_trim_to_start_end_times(ds=data_structure,
                                                                              start=start,
                                                                              end=end,
                                                                              time_bin=time_bin,
                                                                              id_col=id_col,
                                                                              ...
                                                                              **logging_kwargs)
        # set to measurement datetime if it had a start/stop or use original value
        dt_index: str = 'measurement_datetime' if isinstance(dt_index, dict) else dt_index
        
        # set start and end to None since they have already been integrated.
        start = None
        end = None
    ...
    if dt_index is None:
        logm(message='Inferring datetime columns/index', **logging_kwargs, messageLevelName='DEBUG')
    ...
    logm(f'resampling: {cols_to_agg}', **logging_kwargs, messageLevelName='DEBUG')
...
def _auto_resampler(data_structure: pd.DataFrame,
                    ...
                    child: bool = False,
                    **logging_kwargs):
    # debug_inputs(function=_auto_resampler, kwargs=locals(), dump_fp='_autor_reample.p')
    if isinstance(id_col, str):
        levels: np.ndarray = data_structure[[]].reset_index(level=id_col)[id_col].unique()
        ulevels: int = len(levels)

        if ulevels <= max_batch_size:
            pass
        else:
            ds: pd.DataFrame = data_structure.reset_index(level=id_col)

            num_batches: int = int(ulevels / subjects_per_batch)

            logm(message=f'Splitting resampling job into {num_batches} to expedite resampling', **logging_kwargs, messageLevelName='INFO')
            
            auto_resample_kwargs: List[dict] = []
            for i, g in enumerate(np.array_split(levels, num_batches)):
                
                # copy function kwargs
                temp: dict = debug_inputs(function=_auto_resampler, kwargs=locals(), dump_fp=None, skip_engines=False)
               
                # update load_data kwargs
                temp.update({'data_structure': ds[ds[id_col].isin(g)].set_index([id_col], append=True),
                             'child': True})
           
                auto_resample_kwargs.append(temp)
    ...
    for col in cols_to_agg:
        configd: dict = (custom_agg_dict or {}).get(col, {})
        # logm(config, **logging_kwargs)
        dtype = configd.get('ds_dtype', ds_dtype) or _get_column_type(series=data_structure[col], one_hot_threshold=10)
        agg_f = (configd.get('function') or agg_func) or (_numeric_aggregators if dtype in ['int', 'float', 'binary'] else _default_non_numeric_agg)
        if isinstance(agg_f, str):
            agg_f = get_func(agg_f)
        agg_names: list = configd.get('agg_names')
        ld = configd.get('interpolation_limit_direction') or interpolation_limit_direction
        la = configd.get('interpolation_limit_area') or interpolation_limit_area
        m = configd.get('interpolation_method') or interpolation_method
        lim = configd.get('limit') or interpolation_limit
        fv: any = configd.get('fillna_val') or fillna_val
        ...
        try:
            if isinstance(data_structure.index, pd.MultiIndex):
                ...
                tqdm.pandas(desc=f'Resampling {col}', disable=child)
                base: pd.DataFrame = data_structure[col].reset_index(level=dt_index).groupby(level=indexes, group_keys=True)\
                    .progress_apply(lambda x: _resample_and_interpolate_group(df=x, dt_col=dt_index,
...
def omop_nih_race_ethncity(df: pd.DataFrame, ethnicity_col: str = 'ethnicity_concept_id', race_col: str = 'race_concept_id', sex_col: str = 'gender_concept_id') -> pd.DataFrame:
    """
    ...
    if (ethnicity_col in df.columns):
        df[ethnicity_col] = df[ethnicity_col].astype(float).fillna(0).astype(int).replace({38003563: 'Hispanic or Latino',
                                                                                         38003564: 'Not Hispanic or Latino',
                                                                                         0: 'Unknown/Not Reported Ethnicity'})

    if race_col in df.columns:
        df[race_col] = df[race_col].astype(float).fillna(0).astype(int).replace({8527: 'White',
                                                                               38003598: 'Black or African American',
                                                                               45878142: 'Unknnown or Not Reported',
                                                                               0: 'Unknnown or Not Reported',
                                                                               8515: 'Asian',
                                                                               2000000138: 'More Than One Race',
                                                                               38003572: 'American Indian/Alaska Native',
                                                                               38003613: 'Native Hawaiian or Other Pacific Islander'})

    if sex_col in df.columns:
        df[sex_col] = df[sex_col].astype(float).fillna(0).astype(int).replace({8532: 'Female', 8507: 'Male', 0: 'Unknown/Not Reported'})

    return df

def idr_nih_race_ethncity(df: pd.DataFrame, ethnicity_col: str = 'ethnicity', race_col: str = 'race', sex_col: str = 'sex') -> pd.DataFrame:
    """
    Format Dataframe values into NIH standard.
...
def backward_roll(df: pd.DataFrame, group_cols: Union[list, str], window: Union[int, str], on: str,
                  ...
                  label_with_window: bool = False, id_col: Union[str, None] = None, serial: bool = True, **logging_kwargs):
    """
    ...
    """
    if isinstance(id_col, str):
        levels: np.ndarray = df[[]].reset_index(level=id_col)[id_col].unique() if id_col not in df.columns else df[id_col].unique()
        ulevels: int = len(levels)

        if ulevels <= 800:
            pass
        else:
            ds: pd.DataFrame = df.reset_index(level=id_col)

            num_batches: int = int(ulevels / 500)

            logm(message=f'Splitting backward_roll job into {num_batches} to expedite backward rolling', **logging_kwargs, messageLevelName='INFO')
            
            auto_resample_kwargs: List[dict] = []
            for i, g in enumerate(np.array_split(levels, num_batches)):
                
                # copy function kwargs
                temp: dict = debug_inputs(function=backward_roll, kwargs=locals(), dump_fp=None, skip_engines=False)
               
                # update load_data kwargs
                temp.update({'df': ds[ds[id_col].isin(g)].set_index([id_col], append=True)})
           
                auto_resample_kwargs.append(temp)

            return pd.concat([x['future_result'] for x in run_function_in_parallel_v2(function=backward_roll,
                                                                                      kwargs_list=auto_resample_kwargs,
                                                                                      max_workers=min(32, os.cpu_count()),
                                                                                      update_interval=10,
                                                                                      disp_updates=False,
                                                                                      list_running_futures=False,
                                                                                      return_results=True,
                                                                                      log_name='backward_roll',
                                                                                      executor_type='ProcessPool',
                                                                                      debug=serial)], axis=0, sort=False, ignore_index=False)
        
        
    if value_cols is None:
        value_cols: list = [x for x in df.columns.tolist() if x not in ([on] + (group_cols if isinstance(group_cols, list) else [group_cols]))]
        
    if isinstance(last_value, str):
        if last_value == 'XXXlast_rowXXX':
            last_value = df.groupby(group_cols, group_keys=False)[value_cols].tail(1).copy(deep=True).values
            
    assert isinstance(group_cols, (list, str))
    for col in ([on] + (group_cols if isinstance(group_cols, list) else [group_cols])):
        if col not in df.columns:
            df.reset_index(level=col, inplace=True)
    t = df.groupby(group_cols, group_keys=False).rolling(on=on, closed='right', window=window, min_periods=0).apply(agg_func)[[on] + value_cols]
    ...
    if label_with_window:
        t.columns = [f'{x}_{window}' if (x != on) else x for x in t.columns]

    return t.reset_index(level=group_cols)


def _integrate_and_trim_to_start_end_times(ds: Union[pd.Series, pd.DataFrame],
                                           start: Union[pd.Series, pd.DataFrame, pd.Timestamp, None] = None,
                                           end: Union[pd.Series, pd.DataFrame, pd.Timestamp, None] = None,
                                           time_bin: str = '4H',
                                           id_col: str = None,
                                           label_col: str = None,
                                           dt_index: Union[str, dict, None] = None,
                                           start_fill_value: Union[any, None] = None,
                                           end_fill_value: Union[any, None] = None,
                                           value_col: str = None,
                                           serial: bool = True,
                                           subjects_per_batch: int = 500,
                                           max_batch_size: int = 800,
                                           child: bool = False,
                                           **logging_kwargs) -> Union[pd.Series, pd.DataFrame]:
    """
    Add and Trim to end points input data structure.

    Parameters
    ----------
    ds : Union[pd.Series, pd.DataFrame]
        Input data structure. Can be of type dataframe or series.
    start : Union[pd.Series, pd.DataFrame, pd.Timestamp, None], optional
        A time stamp, series of timestamps, or dataframe of timestamps which will be the minimum allowed and inserted into the structure to ensure that is the origin of downstream resampling operations. The default is None.
        If a series or dataframe is used, then the id_col must be present in order to match to the respective subject.
    end : Union[pd.Series, pd.DataFrame, pd.Timestamp, None], optional
        A time stamp, series of timestamps, or dataframe of timestamps which will be the maxiumum allowed and inserted into the structure to ensure that is the end of downstream resampling operations. The default is None.
        If a series or dataframe is used, then the id_col must be present in order to match to the respective subject.
    time_bin : str, optional
        The window in which the data should be resampled if it has a start and end column. The default is '4H'.
    id_col : str, optional
        Column that identifies subjects that is used for start/end alignment. The default is None.
    label_col : str, optional
        Column which has the names for variables if they are in long format. The default is None.
    dt_index : Union[str, dict, None], optional
        The default is None which will infer the time indexes based on data type from the indexes and the columns (if dataframe). If only one time_index is found it will be considered start_col. If two time columns are found, one must contain start/begin and the other must contain stop/end.
        
        alternatively one may specif they start column via dt_index, or may specif the start and end using a dicionary {'start': start_column in datastructure, 'end': end_column in datastructure}
    start_fill_value : Union[any, None], optional
        Value in which the start points should be filled. The default is None.
    end_fill_value : Union[any, None], optional
        Value in which the end points should be filled. The default is None.
    value_col : str, optional
        The column that holds the values (if a pandas dataframe). The default is None. if it is a series it will be auto_assigned to measurement_value
    serial : bool, optional
        Whether a process pool worker should process the chunks in parallel or serially. The default is True.
    **logging_kwargs : TYPE
        key word arguments for the logging module.

    Returns
    -------
    pd.DataFrame
        Trimmed DataFrame with end points appended that has been resampled into 1 time column based on the window.

    """
    # if not child:
    #     debug_inputs(function=_integrate_and_trim_to_start_end_times, kwargs=locals(), dump_fp='trim.p')
    #     raise Exception('stop here')
    assert isinstance(ds, (pd.Series, pd.DataFrame))
    if isinstance(id_col, str):
        if isinstance(ds, pd.DataFrame):
            assert (id_col in ds.index.names) or (id_col in ds.columns)
        elif isinstance(ds, pd.Series):
            assert (id_col in ds.index.names)
            
        levels: np.ndarray = ds[[]].reset_index(level=id_col)[id_col].unique() if id_col not in ds.columns else ds[id_col].unique()
        ulevels: int = len(levels)

        if ulevels <= max_batch_size:
            pass
        else:
            ds: pd.DataFrame = ds.reset_index(level=id_col)

            num_batches: int = int(ulevels / subjects_per_batch)

            logm(message=f'Splitting integrate_and_trim_to_start_end_times job into {num_batches} to expedite process', **logging_kwargs, messageLevelName='INFO')
            
            auto_resample_kwargs: List[dict] = []
            for i, g in enumerate(np.array_split(levels, num_batches)):
                
                # copy function kwargs
                temp: dict = debug_inputs(function=_integrate_and_trim_to_start_end_times, kwargs=locals(), dump_fp=None, skip_engines=False)
               
                # update load_data kwargs
                temp.update({'ds': ds[ds[id_col].isin(g)].set_index([id_col], append=True),
                             'child': True})
           
                auto_resample_kwargs.append(temp)

        
            return pd.concat([x['future_result'] for x in run_function_in_parallel_v2(function=_integrate_and_trim_to_start_end_times,
                                                                                      kwargs_list=auto_resample_kwargs,
                                                                                      max_workers=min(32, os.cpu_count()),
                                                                                      update_interval=10,
                                                                                      disp_updates=False,
                                                                                      list_running_futures=False,
                                                                                      return_results=True,
                                                                                      log_name='resampler-integrate_and_trim_to_start_end_times',
                                                                                      executor_type='ProcessPool',
                                                                                      debug=serial)], axis=0, sort=False, ignore_index=False)
    ...
    # extract dtypes for columns and indicies
    index_dtypes: pd.Series = ds.index.dtypes.astype(str) if isinstance(ds.index, pd.MultiIndex) else pd.Series({ds.index.name: str(ds.index.dtype)})
    index_dtypes = index_dtypes[index_dtypes.index.notnull()]
    column_dtypes: pd.Series = ds.dtypes.astype(str) if isinstance(ds, pd.DataFrame) else pd.Series({ds.name: str(ds.dtype)})
    ...
    if dt_index is None:
        ...
        assert len(dt_indexes) > 0, 'No datetime columns/indexes were found'
        if len(dt_indexes) == 1:
            start_col: str = dt_indexes[0]
            end_col: str = None
        elif len(dt_indexes) == 2:
            if (sum([bool(re.search(r'start|begin', x, re.IGNORECASE)) for x in dt_indexes]) == 1) and (sum([bool(re.search(r'stop|end', x, re.IGNORECASE)) for x in dt_indexes]) == 1):
                start_col: str = [x for x in dt_indexes if bool(re.search(r'start|begin', x, re.IGNORECASE))][0]
                end_col: str = [x for x in dt_indexes if bool(re.search(r'stop|end', x, re.IGNORECASE))][0]
        else:
            assert len(dt_indexes) == 1, f'Multiple datetime columns/indexes were found: {dt_indexes}. Please specify a time index (dt_index) to proceed.'

    elif isinstance(dt_index, (dict, str)):
        start_col: str = dt_index if isinstance(dt_index, str) else dt_index.get('start')
        end_col: str = None if isinstance(dt_index, str) else dt_index.get('end')
    else:
        assert False, f'Invalid input type for variable dt_index. Was expecting None, str, or dict, but found type: {type(dt_index)}'
    
    if isinstance(start, (pd.Series, pd.DataFrame, pd.Timestamp)):
        if isinstance(start, pd.Timestamp):
            stc_col_query: str = "datetime('" + start.strftime("%m/%d/%Y %H:%M:%S") + "')"
        else:
            start = (start.rename('start') if isinstance(start, pd.Series) else start).reset_index(drop=False)
            assert 'start' in start.columns
            assert id_col in start.columns
            
            assert start[id_col].notnull().all()
            assert not start[id_col].duplicated().any()
                
            start[[id_col, 'start']].rename(columns={'start': start_col}).to_sql('start', con=conn, index=False)
            
            stc_col_query: str = f'datetime(start.{start_col})'
            start_field_query: str = f',CASE WHEN julianday(ds.{start_col}) < julianday(start.{start_col}) THEN {stc_col_query} ELSE datetime(ds.{start_col}) END as {start_col}'
            start_join: str = f'INNER JOIN start on start.{id_col} = ds.{id_col}'
    else:
        stc_col_query: str = None
        start_field_query: str = ''
        start_join: str = ''
            
    if isinstance(end, (pd.Series, pd.DataFrame, pd.Timestamp)):
        if isinstance(end, pd.Timestamp):
            end_col_query: str = "datetime('" + end.strftime("%m/%d/%Y %H:%M:%S") + "')"
        else:
            end = (end.rename('start') if isinstance(end, pd.Series) else end).reset_index(drop=False)
            assert 'end' in end.columns
            assert id_col in end.columns
            
            assert end[id_col].notnull().all()
            assert not end[id_col].duplicated().any()
                
            end[[id_col, 'end']].rename(columns={'end': end_col if isinstance(end_col, str) else start_col}).to_sql('end', con=conn, index=False)
            
            end_col_query: str = f'datetime(end.{end_col if isinstance(end_col, str) else start_col})'
            end_field_query: str = f',CASE WHEN julianday(ds.{end_col if isinstance(end_col, str) else start_col}) > julianday(end.{end_col if isinstance(end_col, str) else start_col}) THEN {end_col_query} ELSE datetime(ds.{end_col if isinstance(end_col, str) else start_col}) END as {end_col if isinstance(end_col, str) else start_col}'
            end_join: str = f'INNER JOIN end on end.{id_col} = ds.{id_col}'
            
    else:
        end_col_query: str = None
        end_field_query: str = ''
        end_join: str = ''
        
    # consolidate start/end field queries if there is only one time index
    if (isinstance(end_col_query, str) and isinstance(stc_col_query, str) and not isinstance(end_col, str)):
        end_field_query: str= ''
        start_field_query: str = f', ds.{start_col}'
        
        
    time_filter: str = ('' if (pd.isnull(end_field_query) and pd.isnull(start_field_query)) else
                        f'''(julianday(ds.{start_col}) <= julianday(end.{end_col})
                        AND
                        julianday(ds.{end_col}) >= julianday(start.{start_col}))''' if (isinstance(end_col_query, str) and isinstance(stc_col_query, str) and isinstance(end_col, str)) else
                        f'julianday(ds.{start_col}) BETWEEN julianday(start.{start_col}) AND julianday(end.{start_col})' if (isinstance(end_col_query, str) and isinstance(stc_col_query, str)) else
                        f'julianday(ds.{start_col}) >= julianday(start.{start_col})' if isinstance(stc_col_query, str) else
                        f'julianday(ds.{start_col}) <= julianday(end.{start_col})' if isinstance(end_col_query, str) else '')
    
    if isinstance(ds, pd.Series):
        if not isinstance(value_col, str):
            value_col: str = 'measurement_value'
        ds = ds.rename(value_col)
        
    ds_stage: pd.DataFrame = ds.reset_index(drop=not any([x in [start_col, end_col, id_col] for x in ds.index.names]))
                                                                                                                                                       
    if time_filter != '':
        
        ds_stage.to_sql('ds', con=conn, index=False)
        
        ds_columns: str = 'ds.' + ', ds.'.join([x for x in ds_stage.columns if x not in [start_col, end_col]])
        # query 1, trim to match
        out_part_1: pd.DataFrame = pd.read_sql(f'''SELECT
                                                       {ds_columns}
                                                       {start_field_query}
                                                       {end_field_query}
                                                  FROM
                                                      ...
                                                      {end_join}
                                                 WHERE
                                                     {time_filter};''', con=conn, parse_dates=[start_col, end_col] if isinstance(end_col, str) else [start_col]).drop_duplicates()
                                                     
        dfs_to_append: List[pd.DataFrame] = []                               
        if isinstance(stc_col_query, str) and isinstance(start, pd.DataFrame):
            
            start[value_col] = start_fill_value
            start.rename(columns={'start': start_col}, inplace=True)
            start[end_col] = (start[start_col] + pd.to_timedelta('1 second')).values
            
            if isinstance(label_col, str):
                
                for label in ds_stage[label_col].unique().tolist():
             
                    start[label_col] = label

                    dfs_to_append.append(start[[id_col, start_col, end_col, label_col, value_col] if isinstance(end_col, str) else [id_col, start_col, label_col, value_col]].copy(deep=True))
            else:
                if isinstance(value_col, str):
                    dfs_to_append.append(start[[id_col, start_col, end_col, value_col] if isinstance(end_col, str) else [id_col, start_col, value_col]].copy(deep=True))
                else:
                    for col in [x for x in ds_stage.columns if x not in start.columns]:
                        start[col] = None
                    dfs_to_append.append(start[start.columns.intersection(out_part_1.columns)].copy(deep=True)) 
                    
                    
          
        if isinstance(end_col_query, str) and isinstance(end, pd.DataFrame):
            
            end[value_col] = end_fill_value
            end.rename(columns={'end': end_col if isinstance(end_col, str) else start_col}, inplace=True)
            if isinstance(end_col, str):
                end[start_col] = (end[end_col] -  pd.to_timedelta('1 second')).values
            ...
            if isinstance(label_col, str):
                ...
                for label in ds_stage[label_col].unique().tolist():
                    ...
                    dfs_to_append.append(end[[id_col, start_col, end_col, label_col, value_col] if isinstance(end_col, str) else [id_col, start_col, label_col, value_col]].copy(deep=True))
            else:
                if isinstance(value_col, str):
                    dfs_to_append.append(end[[id_col, start_col, end_col, value_col] if isinstance(end_col, str) else [id_col, start_col, value_col]].copy(deep=True)) 
                else:
                    for col in [x for x in ds_stage.columns if x not in end.columns]:
                        end[col] = None
                    dfs_to_append.append(end[end.columns.intersection(out_part_1.columns)].copy(deep=True)) 
                                


        if len(dfs_to_append) > 0:
            out_part_1: pd.DataFrame = pd.concat([pd.concat(dfs_to_append, axis=0, sort=False, ignore_index=True), out_part_1], axis=0, sort=False, ignore_index=True).sort_values([id_col, start_col], ascending=True)

    else:
        out_part_1: pd.DataFrame = ds_stage
    
    conn.close()
        
    if isinstance(end_col, str):
        tqdm.pandas(desc='Resampling start/end times', disable=child)
        return pd.concat(out_part_1.progress_apply(_resample_rows_with_start_end, value_col=value_col, start_col=start_col, end_col=end_col, time_bin=time_bin, axis=1).values, axis=0, ignore_index=True)
        

    return out_part_1


def _resample_rows_with_start_end(row: pd.Series, value_col: str, start_col: str, end_col: str, time_bin: str) -> pd.DataFrame:
    out = pd.Series(data=row[value_col], index=[row[start_col], row[end_col]]).resample(time_bin, convention='start', label='left', origin='start').ffill().rename(value_col).reset_index(drop=False)\
        .rename(columns={'index': 'measurement_datetime'})
    
    for col in [x for x in row.index if x not in [start_col, end_col, value_col]]:
        out[col] = row[col]
...
if __name__ == '__main__':
    ...
    import copy
    import json
    from Utilities.FileHandling.io import load_data, save_data, get_file_name_components, check_format_series, get_column_names, detect_file_names, check_load_df, get_batches_from_directory
    from Utilities.General.func_utils import get_func, convert_func_to_string, debug_inputs
    from Utilities.ResourceManagement.parallelization_helper import run_function_in_parallel_v2
    from Utilities.PreProcessing.aggregation_functions import _numeric_aggregators, _default_non_numeric_agg
    # from .data_format_and_manipulation import sanatize_columns, notnull, remove_illegal_characters, deduplicate_and_join
    from tqdm import tqdm
    try:
        from pytorch_lightning import seed_everything
    ...
    from Utilities.PreProcessing.standardization_functions import _run_process, resample_pandas, _resample_and_interpolate_group, backward_roll, _integrate_and_trim_to_start_end_times
    
    import pickle
    locals().update(pickle.load(open('trim.p', 'rb')))
    locals().update(pickle.load(open('resample_kwargs.p', 'rb')))
    # locals().update(pickle.load(open('run_process.p', 'rb')))
    
    # ds[['subject_id']].drop_duplicates().merge(cohort[['cohort', 'subject_id']], how='left', on='subject_id').cohort.drop_duplicates()
