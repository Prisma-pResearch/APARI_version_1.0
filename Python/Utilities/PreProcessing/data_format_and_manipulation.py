...
def apply_date_censor(censor_date: Union[str, Dict[str, str]],
                      df: pd.DataFrame,
                      df_time_index: Union[List[str], str, Dict[str, Union[List[str], str]]],
                      column_preference: str = 'first',
                      endpoint_type: str = 'closed',
                      **logging_kwargs) -> pd.DataFrame:
    """
    Filter pandas dataframe based on one or more columns to conform to a date filter.

    Parameters
    ----------
    censor_date : Union[str, Dict[str, str]]
        This can be a singular date, in which case all information before this cutoff date will be kept.
        If a dictinary, then then keys must include one or two of the following options ['max_date', 'min_date'].
            Where max_date will keep informaiton prior to that and min_date will keep information after that.
    df : pd.DataFrame
        Pandas dataframe to be filtered.
    df_time_index : Union[List[str], str, Dict[str, Union[List[str], str]]]
        This can be a single column, a list of columns, or a dictionary of columns with the keys being one or two of the following options ['max_date', 'min_date'].
    column_preference : str, optional
        How multiple columns should be handled when passed as df_time_index.
        The default is 'first', which will use the first non-null value based on that order.
        Other options include 'max', which will use the maximum value in the column list per row and 'min' which will use the min date in the specified columns in each row.
    endpoint_type : str, optional
        Whether the interval/endpoint is open or closed. The default is 'closed', where the date is included in the interval.
        Alternatively the interval can be open where the date is not included in the interval.

    Returns
    -------
    pd.DataFrame
        Pandas Dataframe filtered based on time criteria.

    """
    # check censor date
    assert isinstance(censor_date, (str, dict))
    if isinstance(censor_date, dict):
        assert all([x in ['max_date', 'min_date'] for x in censor_date.keys()])
    
    # check column preference
    if isinstance(column_preference, str):
        assert column_preference in ['first', 'max', 'min']
    else:
        assert isinstance(column_preference, dict)
        assert all([x in censor_date.keys() for x in column_preference.keys()])
        for _, tv in column_preference.items():
            assert tv in ['first', 'max', 'min']
            
    # check endpoint type
    if isinstance(endpoint_type, str):
        assert endpoint_type in ['open', 'closed']
    else:
        assert isinstance(endpoint_type, dict)
        assert all([x in censor_date.keys() for x in endpoint_type.keys()])
        for kv, tv in endpoint_type.items():
            assert tv in ['open', 'closed']

    
    for filter_type, cs_date in ({'max_date': censor_date} if isinstance(censor_date, str) else censor_date).items():
        
        df_time_index: Union[List[str], str] = (df_time_index if isinstance(df_time_index, dict) else {}).get(filter_type, df_time_index)
        if isinstance(df_time_index, str):
            df_time_index: List[str] = df_time_index.split(',')
            
        logm(message=f'Applying {filter_type} filter of {cs_date} using the {(column_preference.get(filter_type) if isinstance(column_preference, dict) else column_preference)} of {df_time_index} and a {(endpoint_type.get(filter_type) if isinstance(endpoint_type, dict) else endpoint_type)} interval',
             **logging_kwargs)
        
        if df.shape[0] > 0:
            
            if (column_preference.get(filter_type) if isinstance(column_preference, dict) else column_preference) == 'first':
                filter_col: pd.Series = df[df_time_index].apply(lambda row: coalesce(*pd.to_datetime(row[df_time_index], errors='coerce')).date(), axis=1)
            elif (column_preference.get(filter_type) if isinstance(column_preference, dict) else column_preference) == 'max':
                filter_col: pd.Series = df[df_time_index].apply(lambda row: pd.to_datetime(row[df_time_index], errors='coerce').max().date(), axis=1)
            elif (column_preference.get(filter_type) if isinstance(column_preference, dict) else column_preference) == 'min':
                filter_col: pd.Series = df[df_time_index].apply(lambda row: pd.to_datetime(row[df_time_index], errors='coerce').min().date(), axis=1)
                
            if filter_type == 'max_date':
                if (endpoint_type.get(filter_type) if isinstance(endpoint_type, dict) else endpoint_type) == 'closed':
                    df: pd.DataFrame = df[filter_col <= pd.to_datetime(cs_date).date()]
                elif (endpoint_type.get(filter_type) if isinstance(endpoint_type, dict) else endpoint_type) == 'open':
                    df: pd.DataFrame = df[filter_col < pd.to_datetime(cs_date).date()]
                ...
                if (endpoint_type.get(filter_type) if isinstance(endpoint_type, dict) else endpoint_type) == 'closed':
                    ...
                    df: pd.DataFrame = df[filter_col > pd.to_datetime(cs_date).date()]
    logm(message='returning deep copy of dataframe after date filtering',
         **logging_kwargs)
...
def sanatize_columns(df: Union[pd.DataFrame, list, pd.Series], preserve_case: bool, preserve_decimals: bool) -> pd.DataFrame:
    ...
    if isinstance(df, (pd.DataFrame, dd.DataFrame)):
        ...
        logm(message=f'Column Sanitation is not currently supported for inputs of type {type(df)}', warning=True)
        return df
    ...
    for _, row in table_info.iterrows():
        ...
        if row.COLUMN_NAME not in df.columns:
            ...
            if str(df[row.COLUMN_NAME].dtype) == 'object':
                ...
                else: # update based on pandas version 2 and greater which does not update the column datatype
                    df[row.COLUMN_NAME] = df.loc[:,
                                                 row.COLUMN_NAME].fillna('missingxxx').astype(str)\
                    .replace({'nan': None, '30000000|': None, 'missingxxx': None})\
                    .str.replace(r'^30000000\|', '', regex=True).values
            ...
            if row.DATA_TYPE in ['date', 'datetime', 'time', 'timestamp', 'datetime2', 'timestamp without time zone']:
                ...
                if bool(re.search(r'^1\.', pd.__version__)):
                    ...
                    df[row.COLUMN_NAME] = force_datetime(df.loc[:, row.COLUMN_NAME]).values

                    if row.DATA_TYPE in ['datetime', 'timestamp', 'datetime2']:
                        df[row.COLUMN_NAME] = df.loc[:, row.COLUMN_NAME].dt.strftime("%Y-%m-%d %H:%M:%S").values
                    elif row.DATA_TYPE in ['time']:
                        df[row.COLUMN_NAME] = df.loc[:, row.COLUMN_NAME].dt.strftime("%H:%M:%S").values
                    elif row.DATA_TYPE in ['date']:
                        df[row.COLUMN_NAME] = df.loc[:, row.COLUMN_NAME].dt.strftime("%Y-%m-%d").values

            elif row.DATA_TYPE in ['bigint', 'int', 'tinyint', 'float', 'double', 'decimal', 'integer', 'numeric', 'smallint']:
                # numeric conversion
                if bool(re.search(r'^1\.', pd.__version__)):
                    df.loc[:, row.COLUMN_NAME] = pd.to_numeric(df.loc[:, row.COLUMN_NAME].copy(),
                                                               errors='coerce' if force_type_conformance else 'ignore')
                else:
                    df[row.COLUMN_NAME] = pd.to_numeric(df.loc[:, row.COLUMN_NAME].copy(),
                                                               errors='coerce' if force_type_conformance else 'ignore')

                # int conversion to string to preserve int format
                if row.DATA_TYPE in ['bigint', 'int', 'tinyint', 'integer', 'smallint']:
                    ...
                    if upload_ints_as_float:
                        if bool(re.search(r'^1\.', pd.__version__)):
                            ...
                            df[row.COLUMN_NAME] = pd.to_numeric(df.loc[:, row.COLUMN_NAME], errors='coerce').values
                            try:
                                df[row.COLUMN_NAME] = df.loc[:, row.COLUMN_NAME].astype(int).values
                            except:
                                pass
                    else:
                        if bool(re.search(r'^1\.', pd.__version__)):
                            ...
                            df[row.COLUMN_NAME] = df.loc[:, row.COLUMN_NAME]\
                                .apply(lambda x: str(int(float(x))) if isfloat(x) else None).values
                ...
                if row.DATA_TYPE in ['float', 'double', 'decimal']:
                    if upload_floats_as_float:
                        ...
                        if bool(re.search(r'^1\.', pd.__version__)):
                            ...
                            df[row.COLUMN_NAME] = df.loc[:, row.COLUMN_NAME].apply(lambda x: f'{x:0.3f}' if pd.notnull(x) else None).values
                ...
                if bool(re.search(r'^1\.', pd.__version__)):
                    ...
                    df[row.COLUMN_NAME] = df.loc[:, row.COLUMN_NAME].fillna('missingxxx').astype(str).str.strip().replace({'nan': None, 'missingxxx': None}).values
            ...
            elif row.DATA_TYPE in ['bit', 'binary']:
                if bool(re.search(r'^1\.', pd.__version__)):
                    df.loc[:, row.COLUMN_NAME] = df.loc[:, row.COLUMN_NAME]\
...
def create_dict(col_action_dict: dict, merge_encounters_flag: bool = False, merge_or_case_encounters_flag: bool = False,
    ...
    #     except KeyError:
    #         pass
    for col in col_action_dict.get('grouping', []):
       try:
           del d[col]
       except KeyError:
           logging.warning(f"Column {col} not found in dictionary")
