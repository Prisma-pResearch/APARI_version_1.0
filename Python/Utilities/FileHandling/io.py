...
def load_data(file_path_query: str,
              ...
              censor_date: Union[str, Dict[str, str]] = None,
              censor_df_time_index: Union[List[str], str, Dict[str, Union[List[str], str]]] = None,
              censor_column_preference: str = 'first',
              censor_endpoint_type: str = 'closed',
              ...
              executor_type: str = 'ProcessPool'if multiprocessing.parent_process() is None else 'ThreadPool',
              clean_ids: bool = False,
              func_after_loading: Union[callable, None] = None,
              func_after_loading_kwargs: Union[dict, None] = None,
              convert_ids_to_sparse_int: bool = False,
              desired_types: Union[dict, None] = None,
              ensure_cols: Union[list, None] = None,
              prefix_dict: Union[dict, None] = None,
              **kwargs):
    """
    ...
    **kwargs : TYPE
    ...
    """
    logging_kwargs: dict = {x: kwargs.get(x, False if (x == 'display') else 'DEBUG' if (x == 'messageLevelName') else None) for x in ['log_name', 'log_dir', 'display', 'messageLevelName']}
    already_loaded: bool = False
    if not isinstance(directory, str):
        format_process: bool = True
        if getDataStructureLib(file_path_query)['type'] == 'dataframe':
            df = file_path_query
            already_loaded = True
        else:
            if not skip_logging:
        ...
        for file in file_list:
            # grab _load_file kwargs
            temp = kwargs.copy()
            # copy load_data kwargs
            proj_kwargs: dict = debug_inputs(function=load_data, kwargs=locals(), dump_fp=None, skip_engines=False)
            proj_kwargs.pop('kwargs', None)
            temp.update(proj_kwargs)
            ...
            #              'func_after_loading': func_after_loading,
            #              'func_after_loading_kwargs': func_after_loading_kwargs,
            #              'nested_process': True,
            #              'clean_ids': clean_ids,
            #              'rename_columns': rename_columns,
            #              'combine_date_time_cols': combine_date_time_cols,
            #              'convert_ids_to_sparse_int': convert_ids_to_sparse_int,
            #              'log_name': ((logging_kwargs.get('log_name') + '.') if isinstance(logging_kwargs.get('log_name'), str) else '') + os.path.basename(file)})
        ...
        if len(data_arr) == 0:
            ...
            df = pd.DataFrame(columns=kwargs.pop('usecols', kwargs.get('columns'))) if return_selected_cols_if_empty else None
        else:
        ...
        if getDataStructureLib(df)['type'] == 'dataframe':
            ...
            if (getDataStructureLib(df)['type'] == 'dataframe') and ('dask' not in str(type(df))):
                if (df.shape[0] > 0) and (isinstance(censor_date, str) or isinstance(censor_date, dict)):
                    ...
                    if isinstance(censor_date, dict):
                        if all([x in ['col', 'censor_date'] for x in censor_date.keys()]):
                            dtc: str = censor_date['col']
                            censor_df_time_index = dtc
                            censor_date = censor_date['censor_date']
                            if isinstance(dtc, str):
                                if dtc in df.columns:
                                    if bool(re.search(r'^1\.', pd.__version__)):
                                        df.loc[:, dtc] = check_format_series(ds=df[dtc].copy(deep=True), desired_type='datetime')
                                    else:
                                        df[dtc] = check_format_series(ds=df[dtc].copy(deep=True), desired_type='datetime')
                                
                    df = apply_date_censor(censor_date=censor_date, df=df, df_time_index=censor_df_time_index,
                                           column_preference=censor_column_preference, endpoint_type=censor_endpoint_type, **logging_kwargs)
                if clean_ids:
                    df: pd.DataFrame = _clean_invalid_ids(df=df, convert_ids_to_sparse_int=convert_ids_to_sparse_int)
                
                pids_to_drop: Union[None, List] = kwargs.get('pids_to_drop', None)
                if isinstance(pids_to_drop, list) and (pid in df.columns):
                    if kwargs.get('drop_override', False) or (get_batch_num(file_path_query) < calculate_number_of_batches(os.path.dirname(file_path_query), file_type=get_file_name_components(file_path_query).file_type) - 1):
                ...
                for c, t in desired_types.items():
                    if c in df.columns:
                        if t == 'format_sparse_int':
                            if bool(re.search(r'^1\.', pd.__version__)):
                                df.loc[:, c] = check_format_series(ds=df[c].copy(deep=True), desired_type='str', format_sparse_int=True)
                            else:
                                df[c] = check_format_series(ds=df[c].copy(deep=True), desired_type='str', format_sparse_int=True)
                        else:
                            if bool(re.search(r'^1\.', pd.__version__)):
                                df.loc[:, c] = check_format_series(ds=df[c].copy(deep=True), desired_type=t)
                            else:
                                df[c] = check_format_series(ds=df[c].copy(deep=True), desired_type=t)
                    else:
                        logm(f'The column {c} was not formatted because it was not in the source dataframe', **logging_kwargs, warning=True)
            
            elif isinstance(date_cols, list):
                if len(date_cols) > 0:
                    df = force_datetime(df, date_cols=list(set(date_cols)))
        
            if already_loaded:
                # filter columns
                selected_cols: list = kwargs.pop('usecols', kwargs.get('columns', None)) or df.columns.tolist()
                ...
                if not kwargs.get('skip_column_name_formatting', False):
                    df = sanatize_columns(df, preserve_case=kwargs.get('preserve_case', False), preserve_decimals=kwargs.get('preserve_decimals', False))
                    selected_cols = sanatize_columns(pd.DataFrame(columns=selected_cols),
                                                     preserve_case=kwargs.get('preserve_case', False),
                                                     preserve_decimals=kwargs.get('preserve_decimals', False)).columns.tolist()
                if kwargs.pop('use_col_intersection', False):
                    df = df[df.columns.intersection(selected_cols)]
                else:
                    df = df[selected_cols]
        
            if isinstance(ensure_cols, list):
                df = ensc(df=df, cols=ensure_cols)
        
            if isinstance(prefix_dict, dict):
                for col, prefix in prefix_dict.items():
                    df.loc[df[col].notnull(), col] = prefix + df.loc[df[col].notnull(), col].astype(str)
    
        if (func_after_loading is not None):
            df = func_after_loading(df, **(func_after_loading_kwargs if isinstance(func_after_loading_kwargs, dict) else {}))
...
def save_data(df: any,
              ...
              save_with_error_group: bool = False,
              write_split_on_fly: bool = False,
              **kwargs):
    """
    ...
    file_type: str, optional
        The file type of the output file. The default is None, which will infer the type from the out_path.
    fillna_value: any, optional
        Value to be used to replace None/NaNs in a dataframe. The default is None.
    inside_parallel_process: bool, optional
    ...
    **kwargs
    ...
    """
    if isinstance(out_path, str) and (not isinstance(file_type, str)):
    ...
    if (typedict['type'] == 'dataframe') and (not show_progress_bar):
        ...
        if isinstance(split_into_n_batches, int) or isinstance(split_by_indentifier_batch_dir, str):
            if isinstance(split_by_indentifer_col, str):
                ...
                dfs = split_by_identifier(df=df, split_by_indentifer_col=split_by_indentifer_col, id_dir=id_dir, id_dict=id_dict,
                                          save_with_error_group=save_with_error_group,
                                          split_into_n_batches=split_into_n_batches, batch_num=batch_num,
                                          write_on_fly=write_split_on_fly,
                                          write_kwargs={**kwargs.copy(), **{'out_file_path': out_path, 'index': index, 'file_type': file_type}},
                                          **logging_kwargs)
            elif typedict['cpul']:
...
def _write_file(out_file_path: str, index: bool, file_type: str,
    ...
    if isinstance(out_file_path, str) and ('.' in str(out_file_path)):
        out_file_path: str = out_file_path[:out_file_path.rfind('.')] + file_type

    if not (isinstance(engine, Engine) or isinstance(engine, sq.Connection)):
        logm(message=f'Writting {os.path.basename(out_file_path)}',
             log_name=log_name, log_dir=log_dir, display=display, messageLevelName=messageLevelName)
    ...
    if typedict['type'] == 'dataframe':
        ...
        if isinstance(engine, Engine) or isinstance(engine, sq.Connection):
            ...
            if not isinstance(df, pd.DataFrame):
                df = df.to_pandas()
            kwargs.pop('index', False)
            df.to_pickle(out_file_path, **kwargs)
        elif file_type in ['.parquet']:
            try:
                df.to_parquet(out_file_path, **kwargs)
            except:
                df.fillna('-999XXX999').astype(str).replace({'-999XXX999': None}).to_parquet(out_file_path, **kwargs)
    ...
    if isinstance(save_success_fp, str):
        open(save_success_fp, 'a').close()
    logm(message=f'Done writting {os.path.basename(out_file_path)}', log_name=log_name, log_dir=log_dir, display=display, messageLevelName=messageLevelName)
    return
...
def _load_file(file_path_query: str,
               ...
               skip_column_name_formatting: bool = False,
               messageLevelName: str = 'DEBUG',
               max_query_tries: int = 5,
               **kwargs) -> pd.DataFrame:
    temp = None
    ...
    if isinstance(engine, Engine) or isinstance(engine, omop_engine_bundle) or isinstance(engine, sq.Connection) or isinstance(query_folder, str):
        ...
        counter: int = 0
        while (counter < max_query_tries) and (not pull_success):
            try:
                df = pd.read_sql(sql=sql, con=engine, **kwargs)
                pull_success: bool = True
            except Exception as e:
                
                if bool(re.search('deadlock\s+victim', str(e), re.IGNORECASE)):
                    counter += 1
                    logm(message=f'Query Deadlocked on attempt: {counter}. {"Trying again" if counter < max_query_tries else "Max allowed attemps exceeded, throwing error"}', **logging_kwargs)
                else:
                    raise Exception(e)
    ...
    elif bool(re.search(r'\.csv$|\.txt$|\.tsv$|\.csv.gz$', file_path_query)):

        kwargs['dtype'] = kwargs.get('dtype', object)
        
        if bool(re.search(r'\.csv.gz$', file_path_query)):
        ...
        if 'usecols' in kwargs:
            kwargs['columns'] = kwargs.pop('usecols', None)
        kwargs.pop('parse_dates', False)
        
        try:
            temp = loading_lib.read_parquet(path=file_path_query, **kwargs)
        except ArrowInvalid as e:
            usecols = kwargs.pop('columns', None)
            
            if isinstance(usecols, list):
                schema = parquet.read_schema(file_path_query, memory_map=True)
                schema = pd.DataFrame(({'source_column': name, 'pa_dtype': str(pa_dtype)} for name, pa_dtype in zip(schema.names, schema.types)))
                schema['clean_column'] = sanatize_columns(df=schema.source_column, preserve_case=preserve_case, preserve_decimals=preserve_decimals)
                schema['useable'] = schema.source_column.isin(usecols) | schema.clean_column.isin(usecols)
                
                kwargs['columns'] = schema.source_column[schema.useable]

                if use_col_intersection or (len(usecols) == schema.useable.sum()):
        
                    df = loading_lib.read_parquet(path=file_path_query, **kwargs)
                else:
                    raise Exception(e)
            else:
                raise Exception(e)
    ...
    if isinstance(temp, expected_type):
        try:
            ...
            logm(message=f'Replacing NA values failed with error: {e}', log_name=log_name, log_dir=log_dir, display=display, messageLevelName='WARNING')
            df = temp
        ...
        if load_all_chunks:
            ...
            for i, c in enumerate(df):
                logm(message=f'Loading Chunk {i}', log_name=log_name, log_dir=log_dir, display=display, messageLevelName=messageLevelName)
                out.append(c)
            logm(message='Merging Chunks', log_name=log_name, log_dir=log_dir, display=display, messageLevelName=messageLevelName)
    ...
    if format_identifiers:
        ...
        if bool(re.search(r'^1\.', pd.__version__)):
            ...
            if ((eid in df.columns) and (eid != 'merged_enc_id')):
                df[eid] = df.loc[:, eid].apply(tokenize_id, token_index=-1, ignore_errors=ignore_id_errors).values

            if 'or_case_key_deiden_id' in df.columns:
                df['or_case_key_deiden_id'] = df.or_case_key_deiden_id.apply(tokenize_id, token_index=-1, ignore_errors=ignore_id_errors).values

            if 'or_case_num_deiden_id' in df.columns:
                df['or_case_num_deiden_id'] = df.or_case_num_deiden_id.apply(tokenize_id, token_index=-1, ignore_errors=ignore_id_errors).values

            if isinstance(pid, list):
                for col in pid:
                    df[col] = df.loc[:, col].apply(tokenize_id, token_index=-1, ignore_errors=ignore_id_errors).values
                if isinstance(pid_map, expected_type):
                    df = df.merge(pid_map, how='left', on=pid).drop(columns=pid)
                ...
                df[pid] = df.loc[:, pid].apply(tokenize_id, token_index=-1, ignore_errors=ignore_id_errors).values
    ...
    if isinstance(id_prefix_to_append, str):
        if bool(re.search(r'^1\.', pd.__version__)):
            ...
            df[pid] = f'{id_prefix_to_append}_' + df.loc[:, pid].astype(str)


    if isinstance(pids_to_drop, list) and (pid in df.columns):
        if drop_override or (get_batch_num(file_path_query) < calculate_number_of_batches(os.path.dirname(file_path_query), file_type=get_file_name_components(file_path_query).file_type) - 1):
            logm(message=f'dropping ids from {os.path.basename(file_path_query)}', warning=True,
                 log_name=log_name, log_dir=log_dir, display=display, messageLevelName=messageLevelName)
            df = df[~df[pid].isin(pids_to_drop)]
    ...
    if tag_source and ('source_file' not in df.columns):
        df['source_file'] = f'{os.path.basename(os.path.dirname(file_path_query))}/{os.path.basename(file_path_query)}' if (df.head(1).shape[0] > 0) else None

    if getDataStructureLib(df)['type'] == 'dataframe':
        if not skip_column_name_formatting:
...
def calculate_number_of_batches(dir_path: str, file_type: str = '.csv') -> int:
    """Calculate the number of batches in a given directory."""
    listOfFiles = find_files(directory=dir_path, patterns=r'_[0-9]+\{}$'.format(file_type), recursive=False, regex=True)
...
def check_df(df, allow_empty_files: bool = False) -> bool:
    ...
    if getDataStructureLib(df)['type'] == 'dataframe':
        ...
        if allow_empty_files or ('dask' in str(type(df))):
            return True
...
def upload_table(engine: Engine, dest_table: str, df: pd.DataFrame,
    ...
    for i, chunk in tqdm(zip([get_batch_num(x) for x in failure_files], batches) if len(failure_files) > 0 else enumerate(batches),
                         total=len(batches),
                         disable=not show_upload_progress,
        ...
        try:
            ...
            logm(message=tm, **logging_kwargs)
            ...
            try_count: int = 0
            sucessful_upload: bool = False
            while (try_count < max_deadlock_retries) and (not sucessful_upload):
                try:
                    with engine.begin() as conn:
                        ...
                        sucessful_upload: bool = True
                except Exception as e:
                    if ('deadlocked' in str(e).lower()) and retry_on_deadlock:
                        logm(message=f'Deadlocked Retrying Upload after {retry_interval_sec} seconds', **logging_kwargs)
                        time.sleep(retry_interval_sec)
                        try_count += 1
                    else:
                        try_count = max_deadlock_retries + 1
                        raise Exception(e)
                        
        except Exception as exc:
            logm(message=exc, display=True, error=True, raise_exception=raise_exception, **logging_kwargs)
            if isinstance(failure_fp, str):
                chunk.to_pickle(chunk_failure_fp)
                if debug:
                    stage.to_pickle(chunk_debug_fp)
                logm(message=chunk_failure_fp, **logging_kwargs)
...
def detect_file_names(directory: str, recursive: bool = True, pattern: str = r'_[0-9]+\.csv',
                      omit_file_names: list = None, drop_duplicates: bool = True, regex: bool = True,
                      exclusion_patterns: Union[List[str], None] = None,
                      file_list: Union[List[str], None] = None) -> list:
    r"""
    ...
    """
    files = pd.Series(file_list if isinstance(file_list, list) else
                      find_files(directory=directory,
                                 patterns=pattern,
                                 exclusion_patterns=exclusion_patterns,
                                 recursive=recursive,
                                 regex=regex))\
        .apply(lambda entry: re.sub(pattern, '', os.path.basename(entry)) if regex else get_file_name_components(entry).file_name)
...
def get_column_names(directory: str, pattern: str = r'_0\.csv$', recursive: bool = False, regex: bool = True) -> pd.DataFrame:
    """Produce a list of all of the columns in each csv or parquet file in a given directory."""
    if re.search(r'\.csv$|\.csv\$|\.parquet$|\.parquet\$', pattern if isinstance(pattern, str) else pattern[0]):
        column_lists: list = []
        ...
        for entry in find_files(directory=directory, patterns=pattern, recursive=recursive, regex=regex, agg_results=True):
            ...
            try:
                if re.search(r'\.csv$|\.csv\$', pattern if isinstance(pattern, str) else pattern[0]):
                    column_lists.append(pd.DataFrame({out_fn: pd.read_csv(entry, nrows=0, low_memory=False).columns}))
                elif re.search(r'\.parquet$|\.parquet\$', pattern if isinstance(pattern, str) else pattern[0]):
                    schema = parquet.read_schema(entry, memory_map=True)
                    schema = pd.DataFrame(({'source_column': name, 'pa_dtype': str(pa_dtype)} for name, pa_dtype in zip(schema.names, schema.types)))
                    schema['clean_column'] = sanatize_columns(df=schema.source_column, preserve_case=False, preserve_decimals=False)
                    schema['file_name'] = out_fn
                    column_lists.append(schema)
            except pd.errors.EmptyDataError as e:
                logm(message=f'Unable to Process columns from {entry}. {e}', logLevelName='WARNING')
                pass
    
        return pd.concat(column_lists, axis=0 if bool(re.search(r'\.parquet$|\.parquet\$', pattern if isinstance(pattern, str) else pattern[0])) else 1)
    else:
        raise Exception('Invalid File type to retrieve file names. Currently only .csv and .parquet are supoorted')
...
def _clean_invalid_ids(df: pd.DataFrame,
                       columns: List[str] = ['hospital_account_deiden_id', 'encounter_deiden_id', 'encounter_deiden_id_primary',
                                             'or_case_num', 'patient_deiden_id', 'or_case_num_deiden_id', 'admit_provider_deiden_id',
                                             'discharge_provider_deiden_id', 'attending_dr_deiden_id', 'referring_dr_deiden_id', 'pcp_deiden_id',
                                             'provider_deiden_id', 'old_att_dr_deiden_id', 'new_att_dr_deiden_id', 'lab_id', 'order_provider_deiden_id',
                                             'auth_provider_deiden_id', 'sched_surgeon_dr_deiden_id', 'main_surgeon_dr_deiden_id', 'main_anesthesiologist_dr_deiden_id',
                                             'med_order_num_deiden_id', 'order_num_deiden_id', 'order_num'],
                       convert_ids_to_sparse_int: bool = True) -> pd.DataFrame:
    for col in [x for x in columns if x in df.columns]:
        if str(df[col].dtype) == 'object':
            invalid_idx: pd.Series = df[col].str.contains(r'None$|<NA>$', regex=True, case=False, na=False)
            if invalid_idx.any():
                df.loc[invalid_idx, col] = None
                
            if convert_ids_to_sparse_int and df[col].str.contains('_', na=False, regex=False).any():
                df.loc[:, col] = df.loc[:, col].str.extract(r'_([0-9]+)$', expand=False).values
                
        elif convert_ids_to_sparse_int:
            df.loc[:, col] = check_format_series(df.loc[:, col], desired_type='sparse_int').values

    return df

def _split_by_indentifer(input_df: pd.DataFrame,
                         id_col: str,
                         split_into_n_groups: int,
                         id_dict: dict = None,
                         save_with_error_group: bool = False,
                         write_on_fly: bool = False,
                         write_kwargs: dict = {}):
    """
    ...
    if isinstance(id_dict, dict):
        cum_ids: list = []
        for k, ids in tqdm(id_dict.items(), total=len(id_dict), desc=f'Splitting{" and Writing " if write_on_fly else " "}file into {len(id_dict)} parts'):
            if write_on_fly:
                
                temp_k: dict = copy.deepcopy(write_kwargs)
                out_path = temp_k.pop('out_file_path')
                temp_k.update({'out_file_path': out_path + '_{}'.format(k) + temp_k.get('file_type'),
                               'df': input_df[input_df[id_col].isin(ids)].copy(deep=True)})
                _write_file(**temp_k)
            else:
        ...
        if save_with_error_group and (len(new_ids) > 0):
            out[f"{dt.now().strftime('%Y-%m-%d_%H-%M-%S')}_overflow"] = input_df[input_df[id_col].isin(new_ids)].copy(deep=True)
        else:
        ...
        return id_dict, _split_by_indentifer(input_df=input_df, id_col=id_col, split_into_n_groups=split_into_n_groups, id_dict=id_dict, save_with_error_group=save_with_error_group)


def split_by_identifier(df: pd.DataFrame, split_by_indentifer_col: str, id_dir: str = None, id_dict: dict = None,
                        split_into_n_batches: int = None, batch_num: str = None, save_with_error_group: bool = False,
                        write_kwargs: dict = {}, write_on_fly: bool = False, **logging_kwargs):
    """
    ...
    **logging_kwargs : TYPE
    ...
    if not isinstance(id_dict, dict):
        ...
        if len(id_files) > 0:
            ...
            for f in id_files:
                try:
                    id_dict[get_file_name_components(f).batch_numbers[-1]] = _load_file(f, eid=None, pid=None, header=None).iloc[:, 0].astype(str).tolist()
    ...
    temp = _split_by_indentifer(input_df=df, id_col=split_by_indentifer_col, split_into_n_groups=split_into_n_batches, id_dict=id_dict, save_with_error_group=save_with_error_group,
                                write_on_fly=write_on_fly, write_kwargs=write_kwargs)
    if isinstance(temp, tuple):
        ...
        for group, ids in temp[0].items():
            pd.Series(ids).to_csv(os.path.join(id_dir,
                                               f'{split_by_indentifer_col}_id_batch_{(batch_num + "_" + str(group)) if isinstance(batch_num, (str, int)) else group}.csv'),
...
def get_batches_from_directory(directory: str,
                               batches: list = None,
                               file_name: str = '^encounters_clean',
                               independent_sub_batches: bool = True,
                               file_type: str = '.csv') -> list:
    """
    ...
    if isinstance(batches, list) and independent_sub_batches:
        ...
        return extract_batch_numbers(file_list=[item for sublist in [find_files(directory=directory,
                                                                                patterns=[r'{}_{}_[0-9]+\{}'.format(file_name, x, file_type), r'{}_{}_chunk_[0-9]+\{}'.format(file_name, x, file_type),
                                                                                          r'{}_{}_subset_[0-9]+\{}'.format(file_name, x, file_type), r'{}_{}\{}'.format(file_name, x, file_type)],
        ...
        return extract_batch_numbers(file_list=find_files(directory=directory,
                                                          patterns=[r'{}_[0-9]+_chunk_[0-9]+\{}'.format(file_name, file_type),
                                                                    r'{}_[0-9]+_subset_[0-9]+\{}'.format(file_name, file_type),
                                                                    r'{}_[0-9]+_[0-9]+\{}'.format(file_name, file_type),
                                                                    r'{}_[0-9]+\{}'.format(file_name, file_type)],
                                                          regex=True),
                                     independent_sub_batches=independent_sub_batches)
