...
def populate_omop_cohort(sql_file: str,
    ...
    """
    # check assertions
    assert subject_id_field in ['visit_detail_id', 'person_id', 'visit_occurrence_id'], f'You must identify the subjects via person, visit_detail, or visit_occurrence ids, however; {subject_id_field} was found'
    ...
    save_data(df=pd.DataFrame({'cohort_definition_name': [cohort_name],
                               ...
                               'definition_type_concept_id': [-9999],  # TODO: Figure out what this means
                               'cohort_definition_syntax': [sql_file if 'SELECT' in sql_file else load_data(sql_file)],
                               'subject_concept_id': [1147332 if subject_id_field == 'visit_occurrence_id' else 1147637 if subject_id_field == 'visit_detail_id' else 1147314],
