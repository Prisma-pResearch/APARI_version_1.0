...
def clean_labs(df: pd.DataFrame,
    ...
    df: pd.DataFrame = check_load_df(df, ds_type='pandas', **logging_kwargs,
                                     desired_types={**{x: 'sparse_int' for x in ['unit_source_concept_id', 'unit_concept_id',
                                                                                 'operator_concept_id', 'measurement_concept_id']},
    ...
    lab_type_cols: list = ['meaurement_concept_id', 'stamped_and_inferred_loinc_code', 'variable_name']
    ...
    unit_lookup_table = check_load_df(os.path.join(resource_dir, 'lab_unit_standard_map_do_not_edit_with_excel.csv'), ds_type='pandas', **logging_kwargs,
                                      desired_types={x: 'sparse_int' for x in ['source_unit_concept_id']})\
        .dropna(subset=['source_unit_concept_id'])
    unit_lookup_table['temp_merge'] = unit_lookup_table.unit_source_value.str.upper().str.strip().astype('category')
    ...
    standard_unit_lookup = check_load_df(os.path.join(resource_dir, 'lab_lookup.xlsx'), sheet_name='standard_lab_units_by_loinc',
        ...
        .drop(columns=['aki_epic_ii_unit', 'aki_epic_frequency', 'idealist_unit', 'idealist_frequency', 'lab_summary_unit',
                       'need_to_check', 'standard_unit'], errors='ignore')\
        .dropna(subset=['lab_type', 'target_unit_concept_id'], how='all')
    standard_unit_lookup.stamped_and_inferred_loinc_code = standard_unit_lookup.stamped_and_inferred_loinc_code.astype('category')
    standard_unit_lookup.target_numerator_type = standard_unit_lookup.target_numerator_type.astype('category')
    standard_unit_lookup.target_denominator_part_1_type = standard_unit_lookup.target_denominator_part_1_type.astype('category')
    standard_unit_lookup.target_denominator_part_2_type = standard_unit_lookup.target_denominator_part_2_type.astype('category')
    ...
    if 'variable_name' in df.columns:
        standard_unit_lookup = standard_unit_lookup.dropna(subset=['lab_type'])\
            .drop(columns=['stamped_and_inferred_loinc_code', 'measurement_concept_id'])\
            .groupby('lab_type')\
            .agg('first')\
    ...
    # UF-Only Impute Loincs and rename columns to omop_standard
    elif (len(df.columns.intersection(['lab_id', 'proc_code', 'stamped_and_inferred_loinc_code'])) == 3):
        ...
        df.lab_id = df.lab_id.astype(float).astype(int).astype(str).astype('category')
        df.proc_code = df.proc_code.astype('category')
        logm(message='Filling in missing LOINCS via Lab IDs', **logging_kwargs)
        missing_2160_mask = df.lab_id.isin(['969', '1526296', '1510379', '3412', '3156']) & df.stamped_and_inferred_loinc_code.isnull()
        if missing_2160_mask.any():
            df.loc[missing_2160_mask, 'stamped_and_inferred_loinc_code'] = '2160-0'
        del missing_2160_mask

        # assign missing POC creatinine loincs
        missing_poc_mask = (df.lab_id == '3028') & df.stamped_and_inferred_loinc_code.isnull()
        if missing_poc_mask.any():
            df.loc[missing_poc_mask, 'stamped_and_inferred_loinc_code'] = '38483-4'
        del missing_poc_mask
        df.stamped_and_inferred_loinc_code = df.stamped_and_inferred_loinc_code.astype('category')
        ...
        # set the merge column for standardizing units
        lab_id_col: str = 'stamped_and_inferred_loinc_code'
    ...
    if 'unit_source_value' in df.columns:
        ...
        df['temp_merge'] = df.unit_source_value.str.upper().str.strip().astype('category')
    ...
    df = df.merge(unit_lookup_table[['temp_unit_concept_id', 'source_unit_name',
                  ...
                  how='left')
    # merge lab standards
    df = df.merge(standard_unit_lookup[standard_unit_lookup.columns.intersection(['stamped_and_inferred_loinc_code',
                                                                                  'lab_type',
    ...
    if debug_conversion:
        return df.loc[nonstandard_mask,
                      ['stamped_and_inferred_loinc_code', 'lab_type', 'unit_source_value', 'temp_unit_concept_id',
    ...
    df.rename(columns={'temp_value_as_number': 'value_as_number',
              ...
              inplace=True)
    df = df[df.columns.intersection(id_cols + ['measurement_datetime', 'stamped_and_inferred_loinc_code', 'value_as_number', 'value_as_concept_id',
                                               'operator_source_value', 'value_as_concept', 'value_source_value', 'unit_source_value', 'source_variable_name',
                                               'range_low', 'range_high', 'result_datetime', 'order_datetime', 'proc_code', 'operator_concept_id', 'variable_name',
                                               'temp_unit_concept_id', 'lab_type', 'unit_concept_id', 'lab_id', 'imputed_unit', 'unit_source_concept_id',
                                               'out_of_range_flag', 'non_standard_unit', 'generate_binary', 'generate_categorical', 'generate_numeric', 'var_abbrev', 'intraop_y_n', 'idr_intraop_y_n'],
                                    sort=False)]
    ...
    if notes_mask.any():
        notes: pd.DataFrame = df.loc[notes_mask,
                                     df.columns.intersection(id_cols + ['result_datetime',
                                                                        'inferred_specimen_datetime', 'proc_code',
                                                                        'stamped_and_inferred_loinc_code',
                                                                        'lab_result', 'lab_name', 'source_row', 'source_file'])]\
            .copy().dropna(subset=['lab_result', 'inferred_specimen_datetime'])
        # df = df[~notes_mask]
        ...
        notes.stamped_and_inferred_loinc_code = notes.stamped_and_inferred_loinc_code.astype(str).replace({'nan': None})
        notes.proc_code = notes.proc_code.astype(str).replace({'nan': None})
        notes.stamped_and_inferred_loinc_code.fillna('xxxmissingxxx', inplace=True)
        grouing_cols = notes.columns.intersection(id_cols + ['inferred_specimen_datetime', 'proc_code', 'stamped_and_inferred_loinc_code', 'lab_name']).tolist()
        notes = notes.fillna('xxxmissingxxx')\
            .groupby(grouing_cols)\
        ...
        if missing_result_dt_mask.any():
            notes.loc[missing_result_dt_mask, 'result_datetime'] = notes.loc[missing_result_dt_mask, 'inferred_specimen_datetime'].values
        del missing_result_dt_mask

        # label name
        notes['note_title'] = (notes.lab_name.fillna('').str.strip()
                               + '|' + notes.proc_code.fillna('').str.strip()
                               + '|' + notes.stamped_and_inferred_loinc_code.fillna('').str.strip()).values

        return df[~notes_mask].copy(), notes.drop(columns=['inferred_specimen_datetime', 'lab_name', 'proc_code',
                                                           'stamped_and_inferred_loinc_code'])\
