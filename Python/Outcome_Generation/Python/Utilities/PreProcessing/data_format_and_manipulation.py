...
def check_format_series(ds: Union[pd.Series], desired_type: any = None, conversion_func: callable = None, **kwargs):
    """
...
def apply_date_censor(censor_date: str, df: pd.DataFrame, df_time_index: str) -> pd.DataFrame:
    """
    ...
    df_time_index : str
        The column in the df to filter by.
...
def sanatize_columns(df: pd.DataFrame, preserve_case: bool, preserve_decimals: bool) -> pd.DataFrame:
...
def stack_df(source_df: pd.DataFrame, index_cols: list, value_col: str, label_col: str) -> pd.DataFrame:
    ...
    """
    t = source_df.groupby(index_cols + [label_col]).agg({value_col: _delimeter_agg})[value_col].unstack()
...
def coalesce(*values):
    ...
    return next((v for v in values if (pd.notnull(v).any() if (isinstance(v, Iterable) and (not isinstance(v, (str, dict)))) else pd.notnull(v))), None)
