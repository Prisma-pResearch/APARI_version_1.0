...
import numpy as np
from Phenotyping.utils import mdrd_fun, eGFR_fun
...
def p05_find_final_class(row):
    ...
    MAY NEED FLOWCHART
    '''
    if row['finalicd910'] == 1:
        if row['esrd_admin_flag'] == 1:
            if row['kidneytransplant_admin_flag'] == 1 and row['kidney_date'] >= row['esrd_date']:
        ...
        if row['kidneytransplant_admin_flag'] == 1:
            return p05_find_subgroup('CKD after kidney transplant by Medical History', row['aki_admin_flag'], row['aki_date'], row['admit_date'], row['admission_creatinine'], row['mdrd'])
    ...
    if row['finalicd910'] == 0 and row['insufficient_data_flag'] == 1:
...
def p05_find_ref_method(row):
    ...
    -----
    '''
    if row.reference_creatinine is np.nan:
        return np.nan
    if row.admission_creatinine == row.reference_creatinine:
        return 'admission_creatinine'
    elif row.min_7_days == row.reference_creatinine:
        return 'min_7_days'
    elif row.medium_8_365_days == row.reference_creatinine:
        return 'medium_8_365_days'
    elif row.mdrd == row.reference_creatinine:
        return 'mdrd'
    else:
        return np.nan
...
def p05_ckd_class_and_egfr_staging(inmd_dir: str, race_correction: bool, pid: str, eid: str, batch: int):
    '''
    This function is used to determine CKD classes and G-stage of CKD if having CKD.
    ...
    '''
    encounter = load_data(os.path.join(inmd_dir, 'encounter_creatinine_parameters', 'encounter_creatinine_parameters_{}.csv'.format(batch)),
                          parse_dates=['admit_datetime', 'birth_date', 'kidneytransplant_code_date', 'kidneytransplant_cpt_date',
                                       'esrd_code_date', 'esrd_cpt_date', 'aki_code_date', 'aki_cpt_date'],
                          pid=pid, eid=eid)
    ...
    encounter['mdrd'] = encounter[['age', 'sex', 'race']].apply(lambda x: mdrd_fun(x[0], x[1], x[2], race_correction), axis=1)

    for x in ['kidneytransplant_code_date', 'kidneytransplant_cpt_date', 'esrd_code_date', 'esrd_cpt_date', 'aki_code_date', 'aki_cpt_date']:
        encounter[x] = encounter[x].dt.date
    encounter['kidney_date'] = encounter[['kidneytransplant_code_date', 'kidneytransplant_cpt_date']].apply(lambda x: max(x), axis=1)
    encounter['esrd_date'] = encounter[['esrd_code_date', 'esrd_cpt_date']].apply(lambda x: max(x), axis=1)
    encounter['aki_date'] = encounter[['aki_code_date', 'aki_cpt_date']].apply(lambda x: max(x), axis=1)

    encounter['final_class'] = encounter.apply(p05_find_final_class, axis=1)
    ...
    encounter.loc[con, 'reference_creatinine'] = encounter.loc[con, ['admission_creatinine', 'min_7_days', 'medium_8_365_days', 'mdrd']].apply(lambda x: np.min(x), axis=1)
    encounter.loc[~con, 'reference_creatinine'] = encounter.loc[~con, ['admission_creatinine', 'min_7_days', 'medium_8_365_days']].apply(lambda x: np.min(x), axis=1)
    encounter['method'] = encounter.apply(p05_find_ref_method, axis=1)

    encounter['egfr'] = np.nan
    con = (encounter['method'].notnull()) & (encounter['method'] != 'mdrd')
    encounter.loc[con, 'egfr'] = encounter.loc[con, ['age', 'sex', 'race', 'reference_creatinine']].apply(lambda x: eGFR_fun(x[0], x[1], x[2], x[3], race_correction), axis=1)
    encounter['egfr_staging'] = np.nan
    con = encounter['final_class_num'] == 1
    encounter.loc[con, 'egfr_staging'] = encounter.loc[con, 'egfr'].apply(lambda x: p05_get_egfr_stage(x))
    encounter['ckd'] = encounter['final_class'].apply(p05_get_CKD_class)
    ...
    if not os.path.exists(os.path.join(inmd_dir, 'encounter_ckd')):
        os.makedirs(os.path.join(inmd_dir, 'encounter_ckd'))

    encounter.to_csv(os.path.join(inmd_dir, 'encounter_ckd', 'encounter_ckd_{}.csv'.format(batch)), index=False)
    encounter_no_esrd.to_csv(os.path.join(inmd_dir, 'encounter_ckd', 'encounter_ckd_noesrd_{}.csv'.format(batch)), index=False)
