...
def p03_add_prev_creatinine_and_egfr_flags(inmd_dir: str, eid: str, pid: str, batch: int, race_correction: bool, version: int, **logging_kwargs):
    """
    Add previousCreatinine Flag as indicator for patient previous creatinine taken and generate patient eGFR parameters.
    ...
    4. Add uncertain_ckd flag column based on the following condition: If finalCodeFlag = 1 and esrd_admin_flag, kidneyTransplant_admin_flag, and ckd_admin_flag = 0 or If finalCodeFlag = 0 and PreviousCreatinineFlag = 1, label uncertain_ckd column as 1. Else label as 0.
    5. Add 3 main areas of creatinine value
        * Locate encounter ids that do not have two eGFR at least 90 days apart as insufficient information and locate encounter ids that finalCodeFlag = 0. Label those rows for insufficient_data_flag = 1.
    ...
    None
    """
    encounter = check_load_df(os.path.join(inmd_dir, 'encounter_admin_flags', 'encounter_admin_flags_{}.csv'.format(batch)),
                              desired_types={'admit_datetime': 'datetime', 'birth_date': 'datetime'},
                              pid=pid, eid=eid, preserve_case=True, dtype=None, **logging_kwargs)
    ...
    ###find the first creatinine
    first_cr = creatinine.copy(deep=True).merge(encounter[[eid, 'admit_datetime']], on=eid, how='inner')
    first_cr = first_cr[first_cr['inferred_specimen_datetime'].dt.date >= first_cr['admit_datetime'].dt.date]
    first_cr = first_cr.sort_values([eid, 'inferred_specimen_datetime'])
    first_cr = first_cr.drop_duplicates([eid])
    first_cr = first_cr.rename(columns={'lab_result': 'first_creatinine',
                                        'inferred_specimen_datetime': 'first_creatinine_datetime'})
    
    creatinine.drop(columns=[eid], errors='ignore', inplace=True)
    ...
    if previous_creatinine.shape[0] == 0:
        previous_creatinine = pd.DataFrame(columns=previous_creatinine.columns.tolist() + ['row_egfr'])
    else:
        previous_creatinine.loc[:, 'row_egfr'] = previous_creatinine[['sample_age', 'sex', 'race', 'lab_result']]\
            .apply(lambda x: eGFR_fun(x[0], x[1], x[2], x[3], race_correction, version), axis=1).copy()

    # add uncertain ckd flag
    encounter['uncertain_ckd'] = 0
    con1 = (encounter['finalCodeFlag'] == 1) & (encounter['esrd_admin_flag'] == 0) & (encounter['kidneyTransplant_admin_flag'] == 0) & (encounter['ckd_admin_flag'] == 0)
    con2 = (encounter['finalCodeFlag'] == 0) & (encounter['PreviousCreatinineFlag'] == 1)
    encounter.loc[con1 | con2, 'uncertain_ckd'] = 1

    # add two egfr information
    previous_creatinine['admit_date'] = None if previous_creatinine.shape[0] == 0 else previous_creatinine['admit_datetime'].dt.date
    previous_creatinine['specimen_date'] = None if previous_creatinine.shape[0] == 0 else previous_creatinine['inferred_specimen_datetime'].dt.date
    data = previous_creatinine if previous_creatinine.shape[0] == 0 else previous_creatinine[previous_creatinine['admit_date'] > previous_creatinine['specimen_date']]
    ...
    con = (encounter[eid].isin(sufficient_creatinine[eid])) & (encounter['finalCodeFlag'] == 0)
    encounter.loc[con, 'insufficient_data_flag'] = 0
    con = (encounter['finalCodeFlag'] == 0) & (encounter['insufficient_data_flag'].isnull())
    ...
    con = (encounter['insufficient_data_flag'] == 0) | ((encounter['insufficient_data_flag'] != 1) & (
        encounter['esrd_admin_flag'] == 0) & (encounter['ckd_admin_flag'] == 0) & (encounter['kidneyTransplant_admin_flag'] == 0))
    ...
    save_data(df=encounter.merge(first_cr[[eid, 'first_creatinine', 'first_creatinine_datetime']],
                                on=[eid], how='left'),
              out_path=os.path.join(inmd_dir,
                                    'encounter_egfr_flags',
                                    'encounter_egfr_flags_{}.csv'.format(batch)),
              index=False, **logging_kwargs)
    save_data(df=previous_creatinine,
              out_path=os.path.join(inmd_dir,
                                    'ckd_row_egfr',
                                    'ckd_row_egfr_{}.csv'.format(batch)),
              index=False, **logging_kwargs)


if __name__ == '__main__':
    # p03_add_prev_creatinine_and_egfr_flags(inmd_dir=inmd_dir, batch=batch, race_correction=race_correction, version=version, eid=eid, pid=pid)
