DROP TABLE IF EXISTS IC3_Inpatient_Pipeline.dbo.IC3_DRUG_LOOKUP_TABLE_v2
GO
SELECT
	...
	[route_name],
	[route_category]
INTO IC3_Inpatient_Pipeline.dbo.IC3_DRUG_LOOKUP_TABLE_v2
FROM
(
 SELECT
 ...
 FROM
 	...
 	INNER JOIN VOCAB.concept A on CA.ancestor_concept_id = A.concept_id
 	INNER JOIN VOCAB.concept D on CA.descendant_concept_id = D.concept_id
 	INNER JOIN VOCAB.DRUG_STRENGTH DS on (DS.drug_concept_id = D.concept_id
 												 AND
 												 DS.ingredient_concept_id = A.concept_id)
 	LEFT JOIN VOCAB.CONCEPT_RELATIONSHIP CR on (CR.concept_id_1 = D.concept_id
 													   AND
 													   Cr.relationship_id LIKE '%has dose form')--= 'RxNorm has dose form')
 	LEFT JOIN VOCAB.CONCEPT DF on DF.concept_id = CR.concept_id_2
 	INNER JOIN VOCAB.CONCEPT NU on NU.concept_id = COALESCE(DS.amount_unit_concept_id, DS.numerator_unit_concept_id)
 	LEFT JOIN VOCAB.CONCEPT DU on DU.concept_id = DS.denominator_unit_concept_id
	LEFT JOIN dbo.IC3_Variable_Lookup_Table_v3_beta l on (l.concept_id = DF.concept_id AND l.concept_class_id = 'Dose Form')
 WHERE
 	CA.ancestor_concept_id IN (SELECT
 									ing.concept_id
 								FROM
 									VOCAB.CONCEPT atc
 									INNER JOIN VOCAB.CONCEPT_ANCESTOR ca on ca.ancestor_concept_id = atc.concept_id
 									INNER JOIN VOCAB.CONCEPT ing on ing.concept_id = ca.descendant_concept_id
 								WHERE
 	...
 	AND
 	D.concept_id NOT IN (SELECT descendant_concept_id FROM VOCAB.CONCEPT_ANCESTOR WHERE ancestor_concept_id = 21604181) -- Anesthetics ATC Class
 UNION
 ...
 WHERE
     ...
     (
 		...
 		CA.ancestor_concept_id IN (SELECT
 									...
 									AND
 									ing.concept_id NOT IN (SELECT descendant_concept_id FROM VOCAB.CONCEPT_ANCESTOR WHERE ancestor_concept_id = 21605145) -- Antiinfectives
 									AND
 									ing.concept_id NOT IN (SELECT descendant_concept_id FROM VOCAB.CONCEPT_ANCESTOR WHERE ancestor_concept_id = 35807331) -- Steriod
 									AND
 									ing.concept_id NOT IN (SELECT descendant_concept_id FROM VOCAB.CONCEPT_ANCESTOR WHERE ancestor_concept_id = 21604254) -- OPIOID
 									)
 	)
 --DIURETICS
 UNION
