...
def check_complete(dir_dict: dict, batch_list: list = None, search_clean: bool = True, phenotyping_flag: bool = False,
                   split_into_n_batches: int = None, files_list: list = None, raise_exception: bool = True, source_dir_key: str = 'source_dir',
                   dest_dir_key: str = 'final_data',
                   initial_check: bool = False, pre_run_check: bool = False, essential_file_name: str = '^encounters',
    ...
    **logging_kwargs : TYPE
    ...
    if isinstance(split_into_n_batches, int):
        completed: pd.DataFrame = found_files.groupby(['file_name', 'batch_num']).agg({'file_type': 'count'})\
    ...
    if initial_check:
        ...
        completed = completed.groupby(['file_name', 'batch_num'])['count'].sum().reset_index(drop=False)
    ...
    def process_problem_df(df: pd.DataFrame) -> pd.DataFrame:
        ...
        for f in problems.file_name.unique().tolist():
            missing_batches: list = list(set(batch_list) - set(found_files.query(f'file_name == "{f}"').batch_num.dropna().unique().tolist()))
