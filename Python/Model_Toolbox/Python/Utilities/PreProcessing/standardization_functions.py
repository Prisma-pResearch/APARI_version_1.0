...
import copy
import json
from ..FileHandling.io import load_data, save_data, get_file_name_components, check_format_series
...
def _get_column_type(series: pd.Series, one_hot_threshold: int, downcast_floats: bool = False):
    ...
    if isinstance(series, pd.DataFrame):
        return series.apply(_get_column_type, one_hot_threshold=one_hot_threshold, downcast_floats=downcast_floats, axis=0)
...
def _auto_resampler(data_structure: pd.DataFrame,
    ...
    for col in cols_to_agg:
        ...
        if isinstance(data_structure.index, pd.MultiIndex):
            ...
            base: pd.DataFrame = data_structure[col].reset_index(level=dt_index).groupby(level=indexes)\
...
def backward_roll(df: pd.DataFrame, group_cols: Union[list, str], window: Union[int, str], on: str,
    ...
    """
    t = df.groupby(group_cols).rolling(on=on, closed='right', window=window, min_periods=0).apply(agg_func)

    if value_cols is None:
        value_cols: list = [x for x in t.columns.tolist() if x != on]

    lv_bf_shift: pd.DataFrame = t.groupby(group_cols)[value_cols].tail(1)
    lv_bf_shift.index = t.groupby(group_cols)[value_cols].head(1).index

    t.loc[:, value_cols] = t.groupby(group_cols)[value_cols].shift(shift)

    t.loc[t.groupby(group_cols)[value_cols].tail(1).index, value_cols] = last_value

    fv_af_shift = t.groupby(group_cols)[value_cols].head(1)
    ...
    t.loc[:, value_cols] = t.groupby(group_cols)[value_cols].ffill()
